...
- **테스트 데이터:** `/home/pia/mou/staging/tmp_data_2` 내 영상
...
| 항목 | 값 |
|------|-----|
| 영상 경로 | `/home/pia/mou/staging/tmp_data_2` |
| 영상 파일 수 | **26개** (mp4 등) |
...
경로 기준: `/home/pia/mou/staging/tmp_data_2` (아래는 상대 경로). **길이** = 재생 시간(초), **파일 크기** = MB.
...
PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --max-videos 6 --no-yolo

# YOLO 포함 시 (localhost:8001 서버 필요)
PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --yolo-url http://localhost:8001

# worker 값별 비교 (추출 시간만, 1/2/4 병렬)
PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --workers 1 2 4 --no-yolo
```
...
`/home/pia/mou/staging/tmp_data_2` 영상으로 **interval 기반** 이미지 추출 실측.  
...
- 벤치 재실행: `PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --max-videos 6 --no-yolo --workers 1 2 4` (전 프리셋) 또는 특정 프리셋만 `--preset-names interval_0.5s_q90 interval_1s_q85`.
...
PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --max-videos 6 --no-yolo --workers 1 2 4

# 특정 프리셋만 (시간 절약)
PYTHONPATH=src python3 scripts/staging_video_extract_yolo_bench.py --video-dir /home/pia/mou/staging/tmp_data_2 --max-videos 6 --no-yolo --workers 1 2 4 --preset-names interval_0.5s_q90 interval_1s_q85
```
...
- **예시 프롬프트:**  
  “`/home/pia/mou/staging/tmp_data_2` 영상으로 프레임 추출 + YOLO-World 검출할 때, 처리 시간과 검출 품질 균형을 위해 `max_frames_per_video`, `jpeg_quality`, `confidence_threshold`, `batch_size` 조합을 추천해 주고, 각 파라미터별로 짧은 근거를 달아줘.”
...
1. **테스트 실행**  
   - 영상 경로: /home/pia/mou/staging/tmp_data_2 (문서 §2.1에 파일 목록·길이·크기 있음)  
