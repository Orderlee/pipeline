...
- 테스트 목적: incoming에 tmp_data_2 + source-d 동시 배치 → archive_pending 이동 후 tmp_data_2만 트리거 → tmp_data_2만 처리·archive 이동, source-d은 archive_pending 유지
...
- **스킵**: 수동 순서로 실행 가능
  ...
  3. `QA_DROP_SOURCE=_qa_smoke_source-d bash staging_qa_safe_drop_incoming.sh source-d`
  4. archive_pending/source-d 대기
...
- source-d(source-d) raw_files 0, archive_pending/source-d 유지
