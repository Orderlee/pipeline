...
| incoming | `/home/user/mou/nas_primary/incoming/` | `/home/user/mou/nas_primary/staging/incoming/` |
| archive | `/home/user/mou/nas_primary/archive/` | `/home/user/mou/nas_primary/staging/archive/` |
...
for d in test test2 test3 appdata; do
  echo "--- $d ---"
  ls /home/user/mou/nas_primary/$d | head -2
  echo "count: $(ls /home/user/mou/nas_primary/$d | wc -l)"
done
...
for f in test test2 test3 appdata_all_scenario1 appdata_part1_scenario2 appdata_part2_scenario2; do
  n=$(ls /home/user/mou/nas_primary/archive/$f 2>/dev/null | wc -l)
  [ "$n" -eq 0 ] && echo "✅ archive/$f=0" || echo "🚨 archive/$f=$n (cleanup_stage 필요)"
done
# 기대: 모두 ✅. 🚨 가 있으면:
#   docker run --rm -v /home/user/mou/nas_primary:/nas alpine rm -rf /nas/archive/<folder>
```
...
copy_to_incoming() {
  ...
  docker run --rm -v /home/user/mou/nas_primary:/nas alpine \
...
hardlink_appdata_all() {
  ...
  docker run --rm -v /home/user/mou/nas_primary:/nas alpine sh -c "
...
drop_dispatch() {
  ...
  docker run --rm \
    -v /home/user/mou/nas_primary:/nas \
...
SQL
  ...
  docker run --rm -v /home/user/mou/nas_primary:/nas alpine sh -c "rm -rf /nas/archive/${folder_pattern}"
  ...
  #    /home/user/mou/nas_primary/<folder> 에 있고, incoming 의 사본만 삭제됨 (안전).
  docker run --rm -v /home/user/mou/nas_primary:/nas alpine sh -c "rm -rf /nas/incoming/${folder_pattern}"
}
...
verify_source_intact() {
  for d in test test2 test3; do
    cnt=$(ls /home/user/mou/nas_primary/$d 2>/dev/null | wc -l)
    [ "$cnt" -eq 20 ] && echo "✅ $d=20" || echo "🚨 $d=$cnt (expected 20)"
  done
  cnt=$(ls /home/user/mou/nas_primary/appdata 2>/dev/null | wc -l)
...
- pending dir 존재? `ls /home/user/mou/nas_primary/incoming/.dispatch/pending/`
...
- `ls /home/user/mou/nas_primary/archive/ | grep <keyword>` 로 확인
- `docker run --rm -v /home/user/mou/nas_primary:/nas alpine rm -rf /nas/archive/<folder>`
