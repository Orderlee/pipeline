...
→ **권장:** Codex MCP 도구를 호출할 때 **`cwd`에 이 repo 루트 경로를 넣어서** 호출하면, 어떤 환경에서든 프로젝트 에이전트가 확실히 로드됩니다. (예: Cursor 에이전트가 `codex` 도구 호출 시 `cwd: "/home/user/work_p/Datapipeline-Data-data_pipeline"` 또는 `workspaceRoot` 전달)
