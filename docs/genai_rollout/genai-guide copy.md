...
> 이미지/텍스트로 영상·이미지를 생성하는 멀티엔진 스튜디오. 결과는 자동으로 데이터
> 파이프라인(MinIO `vlm-raw` + `raw_files`)에 적재되어 학습 데이터로 흘러간다.
>
...
PG (vlm_pipeline_staging):
  ...
  raw_files      — ingest 후 (source_type=genai_source|genai_output, genai_engine)

NAS (호스트 /home/user/mou/staging → 컨테이너 /nas/staging):
  incoming/genai/<YYYY-MM-DD>/<batch_id>/originals/<seq>.<ext>   (입력, txt2video는 없음)
  incoming/genai/<YYYY-MM-DD>/<batch_id>/outputs/<seq>.<ext>     (결과)
  + 각 디렉토리에 _manifest.json (ingest sensor 픽업 트리거)
  → 처리 후 archive/genai/... 로 이동

MinIO (vlm-raw):
  genai/<YYYY-MM-DD>/<batch_id>/outputs/<seq>.<ext>
```
...
POST /genai/batches  (genai 컨테이너)
   ...
   ├─ NAS atomic write: originals/<seq>.<ext> + _manifest.json   (txt2video는 skip)
   └─ adapter.submit()
   ...
   │
   ▼
[기존 ingest sensor] outputs/_manifest.json 픽업
   → MinIO vlm-raw 업로드 + raw_files INSERT + archive 이동
```
