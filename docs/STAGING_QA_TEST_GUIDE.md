...
이미 준비된 폴더가 `staging/tmp_data_2`, `staging/GS건설` 에 있다면 아래 스크립트를 쓰는 게 가장 빠릅니다.
...
- `GS건설` 은 `staging/incoming/GS건설` 에 그대로 남아 있음
...
- `GS건설` 은 `staging/incoming/GS건설` 에 그대로 남아 있어야 함
