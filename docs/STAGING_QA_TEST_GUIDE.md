...
2. `dispatch-agent`가 pending 요청을 제공합니다.
...
- Gemini 인증이 유효해야 함
- staging MinIO `http://10.0.0.36:9002` 접근 가능해야 함
...
```bash
python3 scripts/staging_test_dispatch.py --folder source-d --round 1 --outputs bbox captioning
```
...
예시: `tmp_data_2` 만 실행하고 `source-d` 은 대기 상태로 남기기
...
- `incoming` 에 `source-d`, `tmp_data_2` 두 폴더를 모두 복사
...
SELECT
  CASE
    ...
    WHEN r.raw_key LIKE 'source-d/%' THEN 'source-d'
    ELSE 'other'
  END AS folder_group,
  COUNT(*) AS row_count
FROM raw_files r
...
즉 `source-d` 로 테스트했으면 SQL 에서는 보통 아래처럼 조회해야 합니다.
