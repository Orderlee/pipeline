...
class_crosswalk:
  ...
  # patient/person (sourcej 288장): frame class 사상을 결정하기 전까지
...
Expected (오늘 데이터 기준): `[stamp] ledger: frame 187,994 / 매핑 없음(0단계) / GT 이미지 0 (box 1,558) / crosswalk 제외 {'patient': ..., 'person': ...}` — sourcej 288장이 patient/person **미등재**로 fail-closed 제외되는 것이 올바른 동작이다 (스펙 §9-3).
...
2. **sourcej GT 편입**: patient/person → frame class 사상 결정 후 crosswalk 등재 (`crosswalk_version` 증가) → ledger+gtsync 재실행.
