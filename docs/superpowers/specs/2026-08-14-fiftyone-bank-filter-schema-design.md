...
| `source_unit` | 3 | 58.1% | site-g / v3 / v2 |
...
- **`wave_vs_topk` 의 npz 복원 가능성 미검증.** `wave_<tag>.npz` 에는 wave 쪽 `pred` 만 보이고
  top-k 쪽 예측이 없다. 장래 가지치기를 하려면 이것부터 확인해야 한다. r2 는 삭제하지 않으므로
  **현 계획에는 영향 없음**.
