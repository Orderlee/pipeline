...
이 보고서의 목적은 프롬프트 뱅크 평가 자체를 넘어, **"이런 질문에 답할 수 있는 분석 체계를 FiftyOne 위에 구축한다"**는 것이다. 보고서의 모든 주장은 FiftyOne의 뷰·필드·산점도로 클릭해 검증할 수 있고(§10 인덱스), 전 과정이 원커맨드(`bank_eval.sh`)로 재현된다 — 다음 뱅크 버전(v1.0.9.x, v3.x)이 나오면 같은 분석이 자동으로 나온다. 즉 이 문서는 일회성 분석이 아니라 **뱅크 개발 iteration 을 지원하는 상설 분석 플랫폼의 첫 가동 결과**다.
...
- 2차 후보 7장 중 `폐기물보관장_쓰러짐_20260320_162615_0002` 는 **바닥에 완전히 엎드린 프레임**이라
...
![클래스별 recall 변화](img/bank-report/c1_recall_by_class.png)
...
![falldown 오탐 13장](img/bank-report/s5_falldown_fp.png)
...
![fire 한계곡선](img/bank-report/c2_fire_curve.png)
...
![판정 사분면 (margin_viz × flip)](img/bank-report/s1_margin_quadrant.png)
...
![오탐→정탐 프레임들](img/bank-report/s2_fixed_grid.png)

**바뀐 이유의 분해** (프레임별 rel 점수 분해, `flip_reason` 필드):

![플립 이유 분해](img/bank-report/c3_flip_reasons.png)
...
![smoke 미검출 공백 지도](img/bank-report/s3_gap_smoke.png)
...
![데이터 지도 (emb_viz × GT)](img/bank-report/s4_data_map.png)
...
# 개수/기하 분리 판정이 필요하면 팩토리얼 (docs/prompt-geometry-2026-07-31.md §11)
```
...
docker exec docker-analysis-1 python /tmp/frames_relabel.py normal <stem> …
# 2) 재채점 → 3) 전 스테이지 → 4) 애드혹이던 §4 실험 → 5) 보고서 이미지 9장
docker exec docker-analysis-1 python /workspace/frames_eval.py score
./docker/analysis/bank_eval.sh v1.0.8.0 v1.0.8.4
docker exec docker-analysis-1 python3 /workspace/prompt_geometry.py ablate
docker exec docker-analysis-1 python3 /tmp/merge_factorial.py
docker exec docker-analysis-1 python /tmp/report_charts.py    # c1~c4 + s1~s5
```

산출물: `prompt_geometry.py`(analyze/ablate/gap/viz/flips/guide/slim/report) ·
`merge_factorial.py`(합본·수입 시뮬레이션 + 2⁴ 팩토리얼) ·
`report_charts.py`(보고서 이미지 9장 — s1~s5 는 FiftyOne 앱 스크린샷을 대체한 직접 렌더로,
앱 상태가 URL 로 주소지정되지 않아 캡처가 재현되지 않았기 때문이다) ·
데이터 `/data/fiftyone/sourceh_v2/` · 상세 분석 `docs/prompt-geometry-2026-07-31.md`
