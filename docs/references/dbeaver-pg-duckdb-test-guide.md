...
SELECT duckdb.create_simple_secret(
  ...
  endpoint  := '10.0.0.36:9002',         -- staging MinIO API 포트 (운영은 :9000)
...
| MinIO 엔드포인트 (API) | `10.0.0.36:9002` | **`10.0.0.36:9000`** | §2.4 secret 생성 시 |
...
SELECT duckdb.create_simple_secret(
  ...
  endpoint  := '10.0.0.36:9000',         -- ← staging 9002 가 아님
  url_style := 'path',
  use_ssl   := 'false'
);
```
