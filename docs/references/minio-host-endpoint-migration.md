...
- **10.0.0.36 현재 운영 자원**:
  ...
  - CIFS 공유: `incoming`, `archive`, `staging`, `external`(=`nas`), `AI_data`(=`nas_secondary`), `alarm`
...
- [x] 호스트 `/etc/fstab` 영구 등록 (`10.0.0.51:/volume1/data → /home/user/mou/nas_primary`)
...
- [ ] vlm-dataset 데이터 → `/nas_primary/datasets/` 마이그
...
| /volume1/AI_data (=nas_secondary) | (필요 시 별도 협의)            |
...
| 항목 | 상태 |
|------|------|
| 10.0.0.36의 CIFS 공유들 이전 일정 (incoming/archive/staging/nas/nas_secondary/alarm) | admin 확인 필요 |
...
| 호스트 마운트 | `/home/user/mou/nas_primary` |  |
...
- `~/.claude/projects/.../memory/project_nas_primary.md` — 진행 상태 + 이력
