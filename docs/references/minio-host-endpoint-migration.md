...
- **10.0.0.36 current operational resources**:
  ...
  - CIFS shares: `incoming`, `archive`, `staging`, `external` (=`nas`), `AI_data` (=`nas_secondary`), `alarm`
...
- [x] Host `/etc/fstab` permanent registration (`10.0.0.51:/volume1/data → /home/user/mou/nas_primary`)
...
- [ ] Migrate vlm-dataset data → `/nas_primary/datasets/`
...
| /volume1/AI_data (=nas_secondary) | (separate discussion if needed)  |
...
| Item | Status |
|------|------|
| 10.0.0.36 CIFS shares migration schedule (incoming/archive/staging/nas/nas_secondary/alarm) | Awaiting admin confirmation |
...
| Host mount | `/home/user/mou/nas_primary` |  |
...
- `~/.claude/projects/.../memory/project_nas_primary.md` — progress + history
