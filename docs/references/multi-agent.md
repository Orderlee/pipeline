...
이 문서는 **모델 tier 레벨의 일반 원칙**을 정의합니다. 이 프로젝트의 구체적인 페르소나 명단(누가 무엇을 담당하는지)과 팀 구성은 [`agent-teams.md`](agent-teams.md)에 있습니다 — 이 문서(§2 tier) 위에 도메인 페르소나 레이어가 얹혀 있는 구조입니다.

---

## 1. 아키텍처 개요

**모델 tier 구조**를 따릅니다. 초기에는 Opus / Sonnet / Codex 의 3-tier 였으나, 지금은 값싸고 잦은 런타임 관찰을 위한 Haiku tier 가 더해져 실질적으로 4-tier 로 운영됩니다. tier 위에는 **도메인 페르소나 레이어**가 있고, 각 페르소나(예: `data-engineer`, `ai-engineer`)는 아래 tier 중 하나에 매핑됩니다.

- **리더 (Tier 1)**: Opus — 단독 오케스트레이터 (`cto` 페르소나 / main session)
- **구현 (Tier 2)**: Sonnet — 메인 구현 담당 (여러 구현 페르소나)
- **관찰 (Tier 3)**: Haiku — 값싸고 빠른 런타임 상태 관찰 (`ops-engineer` 페르소나). 읽기·보고 전용
- **교차 검증 (다른 패밀리)**: GPT-5.x (Codex) — 교차 검증자 / 듀얼 작성자

리더는 직접 코드를 많이 작성하지 않습니다. 작업 분해, 라우팅, 통합, 최종 검토에 집중합니다. 서브에이전트는 리더가 정의한 인터페이스와 스키마에 맞춰 결과물을 반환합니다.

> **모델 버전 표기 규칙**: 이 문서는 특정 버전 숫자(예: 4.7)를 박지 않고 tier 별칭(`opus` / `sonnet` / `haiku`)으로만 부릅니다. 실제 서빙 모델은 [`.claude/agents/*.md`](../../.claude/agents/) frontmatter 의 `model:` 필드가 진실이며, 별칭은 그 시점의 최신 모델로 resolve 됩니다. 버전 숫자를 본문에 박으면 곧 stale 해지므로 쓰지 않습니다.
...
- 아키텍처 설계 및 기술 선택 결정
- 각 서브태스크를 적절한 구현 페르소나 또는 Codex에 라우팅
...
- 자기 도메인 밖의 변경이 필요하면 리더에게 에스컬레이션 (페르소나 경계 존중)

### 2.3 GPT-5.x (Codex) — 교차 검증자 (Validator)

- **모델 ID**: `gpt-5.x` (또는 환경에서 사용하는 식별자) — `codex` 페르소나가 Codex MCP 로 호출
- **추론 강도 (reasoning effort)**: 기본 `max`(config.toml·gpt-5.6-sol), 보안·금융·복잡 알고리즘 등 최고난도는 `ultra`, 작은 변경은 `medium`/`high`로 하향. ⚠️ codex MCP `effort` 파라미터는 `medium`/`high`/`xhigh`만 받으므로 `max`는 파라미터 미지정(config 기본값 상속), `ultra`는 `config: {"model_reasoning_effort": "ultra"}` override 로 전달. 자세한 매트릭스는 §3.3 참고. `low`는 검증자 역할에 부적합하므로 사용 금지.
- **컨텍스트**: 검증 대상 코드와 명세만 전달
- **호출 빈도**: 중간 (전체의 약 15–20%)
...
### 2.4 Haiku — 관찰 에이전트 (Observer)

- **모델**: `haiku` 별칭 (frontmatter `model: haiku`)
- **페르소나**: `ops-engineer` — 실행 중인 시스템의 헬스체크, 실패 로그(JSONL) 트리아지, DB 상태 스냅샷, 컨테이너/센서 liveness, MinIO 객체 존재 확인
- **컨텍스트**: 확인 대상 런타임 상태만 (전체 코드베이스 X)
- **호출 빈도**: 값싸고 잦음 (필요할 때마다). 고정 비율 목표 없음
- **제약**: 코드·설정·데이터·컨테이너를 절대 바꾸지 않음 (읽기·보고 전용). 무언가 깨지면 **고치지 않고 담당 페르소나로 라우팅**만 함. 결정은 Tier 1 의 몫

이 tier 는 초기 3-tier 문서에는 없던 추가분입니다. "지금 시스템이 건강한가?"를 Opus/Sonnet 예산을 태우지 않고 싸게 답하기 위해 존재합니다.
...
| 대량 반복 변형 | Sonnet | — |
| 런타임 헬스체크·상태 스냅샷·로그 트리아지 | Haiku | (이상 발견 시) 담당 tier 로 라우팅 |

### 3.2 라우팅 의사결정 흐름

```
요청 도착
  ├─ 런타임 상태 확인만?          → Haiku 단독 (값싸게, 이상 시 담당 tier로 라우팅)
...
`max`가 과한 루틴 리뷰에서 비용·지연이 중요할 때 `effort` 파라미터로 하향합니다. 별도 작업유형 매트릭스는 두지 않고 검증자 재량입니다.
...
명시되지 않은 작업은 기본값 `max`로 처리합니다(파라미터 미지정). 작업 유형이 위 매트릭스 중 두 곳에 걸칠 경우 더 높은 강도를 채택합니다(예: 보안 관련 작은 변경 → `ultra`).
...
리뷰어는 반드시 작성자와 다른 패밀리여야 합니다. (페르소나 레벨에서도 동일 — 모델을 학습·서빙하는 페르소나가 그 모델의 검증까지 겸하지 않습니다. `agent-teams.md` §3.6 참고)
...
읽기 전용 페르소나(`pipeline-explorer`, `deploy-auditor`, `ops-engineer`, `qa-strategist`, `tech-scout`)는 각자 정의된 리포트 형태를 쓰며, 코드 변경이 없으므로 `files_changed`는 비웁니다.
...
Opus 호출이 불가능해지면(쿼터, 비용 한도, 가용성 등), Sonnet 페르소나(주로 `data-engineer` 또는 `dagster-impl`)가 임시 오케스트레이터 역할을 대행합니다.
...
시간이 지나면 "이 영역은 Codex가 더 잘하더라", "저 영역은 Sonnet으로 충분하더라" 같은 데이터가 쌓입니다. 그 데이터로 §3 라우팅 매트릭스를 주기적으로 업데이트하세요. 이 프로젝트의 호출 로그 수집·집계 하네스는 [`agent-teams.md`](agent-teams.md) §6 에 있습니다.
