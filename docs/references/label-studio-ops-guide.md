...
python3 scripts/query_local_duckdb.py --sql "
  ...
  VALUES ('manual-$(date +%s)', 'source-h/falldown', 'vlm-raw', 'completed', 'pending')
"
```
...
python src/gemini/ls_tasks.py \
  --bucket vlm-raw \
  create --prefix source-h/falldown --auto-detect
```
...
python src/gemini/ls_tasks.py \
  ...
  create --prefix sourcej/falldown/image --auto-detect
```
...
|--------|---------------------|
| `sourcej/falldown/clips` | `sourcej__falldown` |
| `sourcei_v2/01_27_collection_data` | `sourcei_v2__01_27_collection_data` |
| `source-h/smoke` | `source-h__smoke` |
...
python src/gemini/ls_tasks.py create --prefix sourcei_v2/01_27_collection_data

# vlm-raw original footage (auto-detect)
python src/gemini/ls_tasks.py --bucket vlm-raw create --prefix source-h/falldown --auto-detect

# vlm-processed images (auto-detect)
python src/gemini/ls_tasks.py create --prefix sourcej/falldown/image --auto-detect

# Renew URL for a specific project
python src/gemini/ls_tasks.py renew --project-name source-h__smoke
