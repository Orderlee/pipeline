# Phase 3: Cycle 1-3 — source-d Both 모드 + 한글 폴더 Dispatch QA 보고서
...
- **한글 폴더명** (`source-d`, `loc-a_site-a-1 B1`, `loc-b_site-d` 등) 처리 검증
...
| 파일명 | 경로 | 크기 | 결과 |
|--------|------|------|------|
| `2026-02-06-135810_...9.mp4` | `loc-a_site-a-1 B1/Gate3 지능형2/` | 961MB | completed |
| `2026-02-06-135815_...13.mp4` | `loc-a_site-a-1 B1/TC2-2/` | 1,012MB | completed |
| `2026-02-20-132722_...1.mp4` | `loc-b_site-d/site-e앞/` | 585MB | duplicate skipped |
...
| 10:55:26 | 중복 스킵: `loc-b_site-d/site-e앞/...mp4` (기존 asset `8c3d9986`, Phase 1에서 ingest 완료) |
...
| 항목 | 값 |
|------|-----|
| source-d raw_files (completed) | 2 |
| source-d raw_files (skipped/dup) | 1 (Phase 1 중복) |
| source-d video_metadata | 2 |
| source-d image_metadata (프레임) | 18 |
| source-d labels | 0 |
| 전체 raw_files | 27 |
...
| 경로 | 상태 |
|------|------|
| `/nas/staging/archive/source-d/` | 2개 비디오 이동 완료 + `_DONE` marker |
| `/nas/staging/archive_pending/source-d/` | 중복 스킵 파일 1개 잔류 |
| `/nas/staging/incoming/source-d/` | 없음 (정리 완료) |
...
|---------------|-------------------|
| `source-d` | `source-d` |
| `loc-a_site-a-1 B1` | `loc-a_site-a-1_b1` |
...
- 그러나 `find_timestamp_pending_by_folder("source-d")` 결과 대상 0건
- 원인: `raw_files.source_unit_name`이 `source-d`이지만, `video_metadata.timestamp_status`가 이미 없거나 `pending`이 아닌 상태
...
- **심각도**: Low
- **설명**: 중복 스킵된 `loc-b_site-d/site-e앞/...mp4`가 `archive_pending`에 남아 있음
