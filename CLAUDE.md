...
#   - prod: `--env-file .env` 명시 → INCOMING/ARCHIVE_HOST_PATH 가 nas_primary 로 정상 resolve (없으면 legacy /home/user/mou/incoming 로 silently revert)
...
| Archive (호스트) | `/home/user/mou/nas_primary/archive` | `/home/user/mou/nas_primary/staging/archive` |
...
- **호스트 ↔ 컨테이너 경로 매핑** (compose의 bind mount) — **현재는 NAS_primary(10.0.0.51, NFS) 로 cutover된 상태** (이전 `/home/user/mou/incoming`·`archive` 직결 경로는 폐기):
  - **PROD**: `/home/user/mou/nas_primary/incoming` → `/nas/incoming`, `/home/user/mou/nas_primary/archive` → `/nas/archive`
  - **STAGING**: `/home/user/mou/nas_primary/staging/incoming` → `/nas/incoming`, `/home/user/mou/nas_primary/staging/archive` → `/nas/archive`
