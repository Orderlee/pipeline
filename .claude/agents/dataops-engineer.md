...
- **PROD** — Postgres `docker-postgres-1` / `vlm_pipeline` / :15433, MinIO `minio-host` 10.0.0.51:9000, NAS `/home/user/mou/nas_primary/{incoming,archive}`.
- **STAGING** — Postgres `pipeline-test-postgres-1` / `vlm_pipeline_staging` / :15432, MinIO :9002, NAS `/home/user/mou/nas_primary/staging/{incoming,archive}`.
...
- **Retention & quota** — storage/retention policy, cleanup of expired presigns/artifacts, and NAS_primary quota: `user` has a quota → do bulk writes via a root container (`docker run --rm -v ...:/dst alpine ...`), not host `cp`/`mkdir`.
...
- **Never delete staging incoming/archive originals** (`/home/user/mou/nas_primary/staging/{incoming,archive}`) without an explicit request — CLAUDE.md forbids it.
