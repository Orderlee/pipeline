...
- **Archive & NAS** — bind mounts on NAS_primary (`INCOMING_HOST_PATH`/`ARCHIVE_HOST_PATH` are truth; `docker exec <ctr> env | grep _HOST_PATH`). `user` has an NFS quota → write big files via a root container, not host `cp`.
