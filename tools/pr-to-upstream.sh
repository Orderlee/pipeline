#!/usr/bin/env bash
# Orderlee/main → upstream-org/dev PR 생성 헬퍼.
#
# 전제:
#   - 현재 repo가 Orderlee 포크이며 원격 'upstream' = upstream-org/Datapipeline-Data-data_pipeline
...
UPSTREAM_REPO="upstream-org/Datapipeline-Data-data_pipeline"
SRC_OWNER="Orderlee"
SRC_BRANCH="main"
...
# 8. PR 생성
TITLE="sync: Orderlee/main → upstream-org/dev ($(date +%Y-%m-%d)) — ${COUNT} commits"
BODY=$(cat <<EOF
