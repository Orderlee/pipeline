...
`ENABLE_EMBEDDING=true`이면 `frame_embedding` / `caption_embedding` 자산이 PE-Core-L14-336 임베딩(1024-d)을 `image_embeddings`(pgvector)에 적재합니다. 이 벡터는 cosine 거리 연산자 `<=>`(또는 L2 `<->`)로 검색합니다.

### 임베딩 종류 (`entity_type`)

| entity_type | 내용 | 비고 |
|-------------|------|------|
| `frame` | 프레임 이미지 임베딩 | cross-modal 텍스트→이미지 검색 대상 |
| `caption` | 캡션 텍스트 임베딩 | 텍스트→캡션 검색 |
| `video` | 비디오 frame-pool 임베딩 | `model_name`에 `/framepool` suffix, partial HNSW 인덱스(009) |

검색 성능을 위해 `entity_type`별 **partial HNSW 인덱스**(`vector_cosine_ops`)를 둡니다 (migration 008, video 009). 단일 통합 인덱스는 `WHERE entity_type='frame'` 같은 filtered cross-modal 쿼리에서 0건을 반환할 수 있어 분리했습니다. recall/latency 튜닝(`hnsw.ef_search`)은 [docs/runbook/hnsw-tuning.md](docs/runbook/hnsw-tuning.md) 참고.

### 검색 방식

raw SQL 예시 (frame 유사 / cross-modal 텍스트→이미지):

```sql
-- 쿼리 벡터 $1 기준 frame 유사 top-k
SELECT e.entity_id, e.image_id, (e.embedding <=> $1::vector) AS cosine_dist
FROM image_embeddings e
WHERE e.entity_type = 'frame' AND e.model_name = $2
ORDER BY e.embedding <=> $1::vector
LIMIT 20;
```

분석 컨테이너의 `fiftyone_pgvector` 헬퍼(노트북에서 import)로 동일 검색을 함수 호출로 수행합니다.

| 함수 | 검색 방향 |
|------|-----------|
| `search_by_text(q, k)` | 텍스트 → 프레임 이미지 (cross-modal, embedding-service `/embed_text`) |
| `search_captions_by_text(q, k)` | 텍스트 → 캡션 |
| `search_videos_by_text(q, k)` | 텍스트 → 비디오 |
| `search_by_image(image_id, k)` | 프레임 → 유사 프레임 |
| `search_similar_videos(asset_id, k)` | 비디오 → 유사 비디오 |

> **Cross-lingual 주의**: PE-Core 텍스트 인코더는 **영어 중심**입니다. 한국어 쿼리도 동작하지만 정렬 품질이 낮습니다(PE-Core 한국어 → degenerate embedding). 최선의 cross-modal 정렬을 위해 한국어 쿼리는 **Gemini로 영어 번역 후** 검색하세요. analysis 컨테이너에는 Gemini SDK/자격이 없으므로 번역은 외부에서 수행해 영어 문자열을 전달합니다.

### 분석 surface (`analysis` profile)

`COMPOSE_PROFILES=analysis`로 `analysis` 컨테이너를 기동하면 임베딩 시각화/유사검색 도구 3종이 뜹니다.

| 도구 | 포트 | 설명 |
|------|------|------|
| JupyterLab | `8888` | `fiftyone_pgvector` 헬퍼로 검색·클러스터·UMAP/PCA/MDS 시각화 (token=`JUPYTER_TOKEN`) |
| FiftyOne App | `5151` | 임베딩 projection 패널 탐색 + SAM3 bbox/캡션 overlay |
| Streamlit | `8501` | `embedding_dashboard.py` — 클러스터 수·projection·거리행렬·nearest 대시보드 (auto-refresh) |
...
주요 테이블은 `src/vlm_pipeline/sql/schema_postgres.sql`에 정의되고, 증분 변경은 `src/vlm_pipeline/sql/migrations/postgres/`(`001`~`008`)로 관리됩니다.
...
- `image_embeddings`는 pgvector 확장이 필요하며(migration 006~008), 확장이 없는 환경에서는 해당 migration이 skip됩니다 (`ENABLE_EMBEDDING`로 gating).
...
│       └── sql/                        # schema_postgres.sql, migrations/postgres/ (001-008)
├── docker/
│   ├── docker-compose.yaml
...
- PostgreSQL은 DuckDB의 단일-파일 write lock 제약이 없으므로, run-coordinator는 단순 `QueuedRunCoordinator`(`max_concurrent_runs`)만 사용합니다 (legacy `duckdb_writer` 등 tag concurrency 제거됨).
