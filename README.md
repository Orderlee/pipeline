...
- **staging**: `/nas/staging/incoming` 기반 검증, `dispatch-agent:8080` polling 기반 dispatch
...
| Dispatch ingress | `.dispatch/pending/*.json` | `dispatch-agent` polling |

핵심 차이:

- **production**은 자동 라벨링을 `.dispatch/pending/*.json` 요청 파일로만 시작합니다.
- **staging**은 `staging_agent_dispatch_sensor`가 `dispatch-agent:8080`에서 pending 요청을 polling해서 시작합니다.
