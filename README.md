...
- **production**: `/nas/incoming` 기반 수집, `dispatch-agent:8080` polling 기반 dispatch + `.dispatch/pending/*.json` fallback
- **staging**: `/nas/staging/incoming` 기반 검증, `dispatch-agent:8081` polling 기반 dispatch
...
│                                                                              │
│ Staging                                                                      │
│   dispatch-agent polling -> staging_agent_dispatch_sensor                    │
...
| Dispatch ingress | `dispatch-agent:8080` polling + JSON fallback | `dispatch-agent:8081` polling |

핵심 차이:

- **production**은 `production_agent_dispatch_sensor`를 기본 ingress로 사용하고, `.dispatch/pending/*.json`은 fallback 경로로 유지합니다.
- **staging**은 `staging_agent_dispatch_sensor`가 `dispatch-agent:8081`에서 pending 요청을 polling해서 시작합니다.
...
| 이름 | 환경 | 설명 |
|------|------|------|
| `production_agent_dispatch_sensor` | production | `dispatch-agent` polling -> `dispatch_stage_job` |
| `dispatch_sensor` | production fallback | `.dispatch/pending/*.json` -> `dispatch_stage_job` |
| `staging_agent_dispatch_sensor` | staging | `dispatch-agent` polling -> `dispatch_stage_job` |
...
- staging은 `dispatch-agent-staging:8081` 연결 상태와 `STAGING_AGENT_POLLING_ENABLED=true` 여부를 먼저 확인합니다.
