...
- **`main` = production**: Dagster `http://10.0.0.10:3030/`, MinIO Console `http://10.0.0.36:9001/`, runtime MinIO endpoint `http://10.0.0.36:9000`
- **`dev` = test**: Dagster `http://10.0.0.10:3031/`, MinIO Console `http://10.0.0.36:9003/`, runtime MinIO endpoint `http://10.0.0.36:9002`
- test는 기존 staging 데이터 plane(`/data/staging.duckdb`, `/home/user/mou/staging/...`)를 재사용하지만, **코드 로직은 production과 동일**합니다.
...
┌──────────────────────────────────────────────────────────────────────────────┐
│ NAS                                                                          │
│  Production host bind: /home/user/mou/incoming, /home/user/mou/archive         │
│  Test host bind:       /home/user/mou/staging/incoming, /home/user/mou/staging/archive │
└───────────────────────────────┬──────────────────────────────────────────────┘
                                │
                                v
┌──────────────────────────────────────────────────────────────────────────────┐
...
│   dispatch-agent polling -> dispatch_agent_sensor                            │
...
| Archive host path | `/home/user/mou/archive` | `/home/user/mou/staging/archive` |
| Runtime MinIO endpoint | `http://10.0.0.36:9000` | `http://10.0.0.36:9002` |
| MinIO Console | `http://10.0.0.36:9001/` | `http://10.0.0.36:9003/` |
...
| 이름 | 환경 | 설명 |
|------|------|------|
| `dispatch_agent_sensor` | prod/test | `dispatch-agent` polling -> `dispatch_stage_job` |
...
- production에서 자동 라벨링은 `dispatch-agent:8080` polling이 기본 ingress입니다 (`DISPATCH_AGENT_POLLING_ENABLED=true` 또는 legacy `PROD_AGENT_POLLING_ENABLED=true`). `.dispatch/pending/*.json`은 fallback으로 유지됩니다.
- test에서는 `dispatch-agent-staging:8081` 연결 상태와 `DISPATCH_AGENT_POLLING_ENABLED=true` 여부를 먼저 확인합니다.
