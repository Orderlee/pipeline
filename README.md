...
| 이름 | 환경 | 설명 |
|------|------|------|
| `production_agent_dispatch_sensor` | prod/test | `dispatch-agent` polling (`/api/production/dispatch/*`) → `dispatch_stage_job` |
...
- test에서는 `dispatch-agent-staging:8081` 연결 상태와 `STAGING_AGENT_POLLING_ENABLED=true` 여부를 먼저 확인합니다.
