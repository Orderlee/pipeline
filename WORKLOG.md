...
- **MVP 재개 v2 (Dual Path + Strict 8) 구현 완료**:
  - `docker/docker-compose.yaml`, `docker/.env` 정렬:
    ...
    - `/nas/datasets/projects` bind mount 추가(`PROJECTS_HOST_PATH=/home/user/mou/nas_192tb/datasets/projects`)
...
- **추가 작업 (오후 세션, 경로/실행 정렬)**:
  - `docker/docker-compose.yaml` 바인드 마운트 fallback 정렬:
    - `PROJECTS_HOST_PATH` fallback을 `/home/user/mou/nas_192tb/datasets/projects`로 통일
...
- **Local/Docker 실행 동등성(Parity) 정리**:
  ...
  - `configs/global.yaml`:
    - `nas_root`: `/home/user/mou/nas_192tb/datasets/projects`
