...
- **변경 통계**:
    - 변경 파일 **80개**, +2164/-617줄.
- **관련 커밋**:
    - `6d65c95a`: feat(genai): 배치 취소 기능 — POST /batches/{id}/cancel + UI 버튼
    - `bc21f67c`: feat(genai): 단일 업로드 한도 20→50 + 메인 페이지 한도 표시 동적화
    - `cb37a613`: feat(genai): bulk 업로드 상한 25→50 (GENAI_MAX_BULK_JOBS 기본값)
    - `746eebde`: build(lint): tracked ruff.toml 추가 — CI·pre-commit·로컬 설정 단일화 (lint 드리프트 근본 차단)
    - `131cb3c6`: style(lint): ruff format 67파일 + E402 noqa 2건 — lint.yml green 복귀
...
### 4. 당일 정리
- **변경 통계**:
    - 변경 파일 **92개**, +15629/-422줄.
- **관련 커밋**:
    - `97c0bdcd`: test(mlops): update post-receive hook test to new direct-ingest contract
    - `1355330d`: docs(mlops): split DVC + MLflow operations runbooks (DVC.md, MLFLOW.md)
    - `ae98769f`: feat(mlops): wire build_trainset 'sources' config → multi-project DVC assemble+freeze (Tier 2 asset)
    - `2e8e9b19`: feat(mlops): freeze_multi_project_trainset — combined COCO → immutable train_dataset_versions
    - `653e51c8`: feat(mlops): assemble_multi_project_coco — combine N pinned DVC datasets (Tier 2 integration)
- **서비스 상태**: 파이프라인 서비스 16개 컨테이너 중 16개 정상 가동.
- **작업 환경**: VSCode
...
### 4. 당일 정리
- **변경 통계**:
    - 변경 파일 **82개**, +2420/-15753줄.
- **관련 커밋**:
    - `31533450`: feat(genai): remaining 카드 '요금제 N개' → 'N번째'(현재 활성 요금제 순번) 표기
    - `07c2835f`: feat(genai): Costs total cost 카드 = 이전+현재 요금제 구매 누적합
    - `a1b64b7e`: feat(genai): remaining 카드 '요금제 N개' → 'N번째'(현재 활성 요금제 순번) 표기
    - `f4ae2e36`: feat(genai): Costs total cost 카드 = 이전+현재 요금제 구매 누적합
    - `87f91388`: perf(analysis): AL 큐 pgvector pushdown — 대용량 OOM/2만 캡 제거
- **서비스 상태**: 파이프라인 서비스 16개 컨테이너 중 16개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **32개**, +1406/-50줄.
- **관련 커밋**:
    - `4b1c3a6e`: merge: origin/dev → main (local only, no deploy) — embed-video + env-backfill + caption-en-embed
    - `77a3a9c4`: feat(genai): Kling duration 3~15초 노출 + 기본/우선 모델 kling-v3
    - `b572870c`: feat(embedding): idle 시 VRAM 자동 해제 (SAM3 idle-unload 패턴 미러)
    - `b5777209`: feat(db): add image_label_annotations table for finalized bbox ground-truth
- **서비스 상태**: 파이프라인 서비스 16개 컨테이너 중 16개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **37개**, +2758/-62줄.
- **관련 커밋**:
    - `71ab263f`: chore(lint): ruff 0.7.4 format drift 정렬 (9 pre-existing 파일, CI lint green)
    - `5b843787`: fix(runbook): pipeline-postgres 포트복원 스크립트 안전화 (Codex BLOCK 3 fatal+4 risk)
    - `aa713fc2`: docs(runbook): 코디네이트 prod 재가동 번들 (임베딩 정식화 + KPI 포트복원)
    - `74979ca1`: feat(analysis): prod 무중단 임베딩 backfill + FiftyOne 기동 스크립트
    - `5b1c1df9`: fix(analysis): Dockerfile 에 embedding_dashboard.py COPY + 8501 EXPOSE
- **서비스 상태**: 파이프라인 서비스 16개 컨테이너 중 16개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **32개**, +5087/-143줄.
- **관련 커밋**:
    - `9745230b`: fix(analysis): download MinIO media locally for FiftyOne (presigned URL filepath was mangled)
    - `ab6c3a6c`: Merge pull request #269 from Orderlee/fix/embed-cleanup
    - `72e96f3a`: fix(embed): FiftyOne mongo sidecar + role default(all whole-image) + healthcheck + ruff format
    - `c4ff949c`: Merge pull request #268 from Orderlee/feature/embed-pgvector-frames
    - `0af5c6a1`: feat(analysis): FiftyOne+JupyterLab analysis container (embedding viz/search)
- **서비스 상태**: 파이프라인 서비스 16개 컨테이너 중 16개 정상 가동.
- **작업 환경**: VSCode
...
- **원인**: 자동 기록이 파일/커밋 나열 중심으로 작성되면 코드, 설정, 문서 변경이 어떤 의도로 묶였는지 드러나지 않음.
- **조치**:
    - 기타 변경을 정리함: `src/gemini/ls_sync.py`, `src/gemini/ls_tasks.py`, `src/gemini/ls_tasks_minio.py`
    - 관련 커밋: `ce063e4e` Merge pull request #266 from Orderlee/fix/ls-renew-minio-guard-token, `9a9899fa` fix(ls): renew --all-projects 가 project 마다 LS access token 재발급 (mid-batch 401 방지), `105dd07e` fix(ls): ls_tasks/ls_sync MinIO 자격 가드에 ALLOW_INSECURE_DEFAULT_CREDS escape-hatch 연결
    - 관련 파일:
      - `src/gemini/ls_sync.py`
      - `src/gemini/ls_tasks.py`
      - `src/gemini/ls_tasks_minio.py`

### 2. 당일 정리
- **변경 통계**:
    - 변경 파일 **3개**, +104/-8줄.
- **관련 커밋**:
    - `ce063e4e`: Merge pull request #266 from Orderlee/fix/ls-renew-minio-guard-token
    - `9a9899fa`: fix(ls): renew --all-projects 가 project 마다 LS access token 재발급 (mid-batch 401 방지)
    - `105dd07e`: fix(ls): ls_tasks/ls_sync MinIO 자격 가드에 ALLOW_INSECURE_DEFAULT_CREDS escape-hatch 연결
...
### 5. 당일 정리
- **변경 통계**:
    - 변경 파일 **65개**, +11598/-10051줄.
- **관련 커밋**:
    - `41f22969`: Merge pull request #265 from Orderlee/dev
    - `f145bd89`: Merge pull request #264 from Orderlee/chore/compose-ls-webhook-escape-hatch-env
    - `f04c8ff9`: chore(compose): ls-webhook environment 에 ALLOW_INSECURE_DEFAULT_CREDS pass-through 추가 (PR #260 follow-up)
    - `7b6ea67b`: Merge pull request #263 from Orderlee/dev
    - `1118f69b`: Merge pull request #262 from Orderlee/fix/ls-webhook-os-import-hotfix
- **서비스 상태**: 파이프라인 서비스 12개 컨테이너 중 12개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **11개**, +3630/-93줄.
- **관련 커밋**:
    - `2119bc7e`: docs: MVP 아키텍처 스냅샷(2026-02-20) + 데이터 플랫폼 Foundation 스프린트 계획
    - `83ea86d2`: Merge pull request #195 from Orderlee/dev
    - `05b2b48e`: Merge pull request #194 from Orderlee/chore/fire-smoke-thresholds
    - `3a19e337`: chore(sam3): fire threshold 0.74 / smoke threshold 0.56 (사용자 튜닝)
    - `af4738c8`: Merge pull request #193 from Orderlee/dev
- **서비스 상태**: 파이프라인 서비스 13개 컨테이너 중 13개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **10개**, +1467/-87줄.
- **관련 커밋**:
    - `2a6831e5`: Merge pull request #157 from Orderlee/dev
    - `be3ed77b`: Merge pull request #156 from Orderlee/fix/ls-image-fallback-classes
    - `cc2da19d`: fix(ls): image mode (bbox) 가 categories 비어있어도 classes 로 fallback
    - `e9fc07b1`: Merge pull request #155 from Orderlee/dev
    - `8959c39d`: Merge pull request #154 from Orderlee/fix/genai-radio-name-collision
- **서비스 상태**: 파이프라인 서비스 12개 컨테이너 중 12개 정상 가동.
- **작업 환경**: VSCode
...
### 6. 당일 정리
- **변경 통계**:
    - 변경 파일 **124개**, +11227/-5698줄.
- **관련 커밋**:
    - `be1c8921`: Merge pull request #147 from Orderlee/dev
    - `c4ecea94`: Merge pull request #115 from Orderlee/chore/deploy-stack-genai-handling
    - `b8210c36`: Merge pull request #146 from Orderlee/feat/docker-image-prune-sidecar
    - `08aaa962`: feat(ops): docker prune script + runbook (Phase 5-D, GHCR 도입 대안)
    - `f3cbc72b`: Merge pull request #145 from Orderlee/dev
- **서비스 상태**: 파이프라인 서비스 12개 컨테이너 중 12개 정상 가동.
- **작업 환경**: VSCode
...
- **변경 통계**:
    - 변경 파일 **40개**, +7229/-506줄.
- **관련 커밋**:
    - `918415e2`: Merge pull request #120 from Orderlee/dev
    - `07682421`: Merge pull request #119 from Orderlee/fix/phase3c-test-runner-pip-cache
    - `d6388720`: fix(test): CI runner pip-cache stale → tests/ 에서 source 파일 직접 import
    - `bcfc39c6`: Merge pull request #118 from Orderlee/feat/phase3c-asset-checks
    - `8d76e9ea`: feat(dq): Phase 3-C — Dagster @asset_check declarative DQ gates (stack-candidates #2)
...
### 6. 당일 정리
- **변경 통계**:
    - 변경 파일 **83개**, +1081/-12237줄.
- **관련 커밋**:
    - `adf62188`: Merge pull request #74 from Orderlee/dev
    - `b54464be`: refactor(db): Phase 3 — purge file-based DuckDB code (PG-only)
    - `ad508df6`: refactor(db): Phase 2 — DuckDBResource → PostgresResource (PG-only cutover)
    - `78f72657`: refactor(sensor): DuckDB-as-PG-bridge removal — Postgres direct
    - `2f0be866`: sec(sql): SQL injection audit fixes — value bindings + sensor allowlist
...
- **변경 통계**:
    - 변경 파일 **45개**, +4439/-62줄.
- **관련 커밋**:
    - `1fe5771f`: feat(agent): wire up sub-agent harness — routing docs, PreToolUse logger, decisions log
    - `7ba197fe`: Merge pull request #73 from Orderlee/dev
    - `d2ebb254`: Merge pull request #72 from Orderlee/feat/vlm-dataset-nfs-migration
    - `d01229fd`: feat(build): vlm-dataset NFS 마이그레이션 (B-narrow)
- **서비스 상태**: 파이프라인 서비스 8개 컨테이너 중 8개 정상 가동.
- **작업 환경**: Cursor, VSCode
...
- **원인**: 자동 기록이 파일/커밋 나열 중심으로 작성되면 코드, 설정, 문서 변경이 어떤 의도로 묶였는지 드러나지 않음.
- **조치**:
    - 파이프라인 코드 변경을 정리함: `src/vlm_pipeline/defs/build/assets.py`, `src/vlm_pipeline/defs/build/sensor.py`
    - 관련 커밋: `c820ec10` Merge pull request #41 from Orderlee/dev, `7c179250` Merge pull request #40 from Orderlee/fix/build-dataset-self-healing, `ce48918d` fix(build): build_dataset self-healing — swallow 제거 + sensor run_key + RetryPolicy
    - 관련 파일:
...
- **관련 커밋**:
    ...
    - `ce48918d`: fix(build): build_dataset self-healing — swallow 제거 + sensor run_key + RetryPolicy
...
- **관련 커밋**:
    ...
    - `abd02e28`: fix(ls_sync,ls_webhook): state.json label_keys 자동 누적 갱신
- **서비스 상태**: 파이프라인 서비스 6개 컨테이너 중 6개 정상 가동.
- **작업 환경**: Cursor, VSCode
...
- **변경 통계**:
    - 변경 파일 **28개**, +3082/-549줄.
- **관련 커밋**:
    - `bc69eb99`: chore(gitignore): 로컬 에이전트·env 템플릿·LS runtime state 추적 제외
    - `b2d0c80e`: docs(classification): vlm-classification 버킷 정책 명문화
    - `fa4a4ce0`: fix(ci): Claude PR Review max-turns 5 → 15
    - `2cdaccf4`: Merge pull request #12 from Orderlee/dev
    - `f01d84e7`: chore(compose): yolo/sam3 를 pipeline-network 에 연결 (dev 스택 공유)
- **서비스 상태**: 파이프라인 서비스 6개 컨테이너 중 6개 정상 가동.
- **작업 환경**: Antigravity, Cursor, VSCode
...
- **조치**:
    ...
    - 포트 `8080`은 `dispatch-agent`가 점유 중이므로 `8084`로 변경
...
- **조치**:
    ...
    - `origin/dev`로 푸시 후 `Orderlee:dev -> upstream-org:dev` PR 생성.
...
- **staging 재테스트용 상태를 완전히 초기화하는 절차를 정리하고 실제로 수행**:
  ...
  - 조치:
    ...
    - staging MinIO endpoint(`10.0.0.36:9002`)의
      - `vlm-raw`
      - `vlm-labels`
      - `vlm-processed`
      - `vlm-dataset`
  ...
  - 운영 기준:
    - 이 초기화는 staging 파이프라인 상태만 비우는 절차이며,
      `/home/user/mou/staging/incoming`, `/home/user/mou/staging/archive` 원본 입력 폴더는 건드리지 않음.
...
- **staging 비용/시간 산정 문서를 만들어 Gemini Pro/Flash, 이미지 추출, YOLO-World 처리량을 정리**:
  ...
  - 포함 내용:
    - `/home/user/mou/staging/incoming` 현재 적재 영상 `38개`, 총 `43.32h`, 총 `22.74GB`
...
- **staging incoming 경로 미인식 문제를 해결하고 실제 ingest 진행까지 확인**:
  - 증상:
    - 호스트에는 `/home/user/mou/staging/incoming` 아래 파일이 계속 이동 중이었지만,
  ...
  - 원인:
    ...
    - 기본 공통 볼륨은 `/home/user/mou/incoming -> /nas/incoming`, `/home/user/mou/archive -> /nas/archive`만 연결하고 있었음.
  - 조치:
    - `dagster-staging`에
...
- **staging `auto_labeling_job`의 Gemini credential 누락을 복구**:
  ...
  - 조치:
    - `docker/.env.staging`에
      ...
      - `GEMINI_GOOGLE_APPLICATION_CREDENTIALS=/app/credentials/your-gcp-project-credentials.json`
...
- **MinIO raw 경로 prefix 정리 및 원인 확인**:
  ...
  - 조치:
    - MinIO 객체를
      - `vlm-raw/2026/03/source-b-event-bucket/...` -> `vlm-raw/source-b-event-bucket/...`
      - `vlm-raw/2026/03/source-c-event-bucket/...` -> `vlm-raw/source-c-event-bucket/...`
      로 이동.
...
- **INGEST MinIO 업로드 재활성화 + raw_key 경로 정책 변경**:
  ...
  - 검증:
    - 신규 샘플 key가 `source-a-rtsp-bucket/...`, `source-b-event-bucket/...` 형태인지 확인.
...
- **Dagster GCS 다운로드 연동 추가**:
  ...
  - 다중 버킷 지원 추가:
    - 대상 버킷: `source-a-rtsp-bucket`, `source-b-202512`
...
- **실데이터 정리 수행 (incoming → archive)**:
  - 대상: `/home/user/mou/incoming/AX지원사업`
  - 결과:
    - 이동: 1596건 → `/home/user/mou/archive/2026/02/AX지원사업`
...
- **INGEST 실운영 장애 대응 및 경로 정렬**:
  - `docker/.env`:
    - `INCOMING_HOST_PATH=/home/user/mou/incoming`로 확정.
...
- **MVP 재개 v2 (Dual Path + Strict 8) 구현 완료**:
  - `docker/docker-compose.yaml`, `docker/.env` 정렬:
    ...
    - `/nas/datasets/projects` bind mount 추가(`PROJECTS_HOST_PATH=/home/user/mou/nas_secondary/datasets/projects`)
...
- **코드/스크립트 정렬**:
  - `update_python_scanner/run_scanner.py`:
    - 기본 루트 하드코딩(`/nas/datasets/projects/cohort-a`) 제거
...
- **검증 결과**:
  ...
  - INGEST:
    - `/test_data/projects/cohort-a/organized_videos/helemet_2` 스캔
...
- **추가 작업 (오후 세션, 경로/실행 정렬)**:
  - `docker/docker-compose.yaml` 바인드 마운트 fallback 정렬:
    - `PROJECTS_HOST_PATH` fallback을 `/home/user/mou/nas_secondary/datasets/projects`로 통일
    - `INCOMING_HOST_PATH` fallback은 `/mnt/nas/incoming` 유지 (fallback 전용)
  - `docker/.env` 최종값 확인:
    - `PROJECTS_HOST_PATH=/home/user/mou/nas_secondary/datasets/projects`
    - `ARCHIVE_HOST_PATH=/home/user/mou/archive`
    - `INCOMING_HOST_PATH=/home/user/mou/incoming`
  - 서비스 재기동:
...
- **Local/Docker 실행 동등성(Parity) 정리**:
  ...
  - `configs/global.yaml`:
    - `nas_root`: `/home/user/mou/nas_secondary/datasets/projects`
...
- **Dagster 파이프라인 구성**:
  ...
  - `nas / cohort-a` SourceAsset 제거 (postgres 폴더 트리와 중복).
...
- **Docker 환경 설정**:
  ...
  - 테스트용 `/home/user/test_data:/test_data` 볼륨 마운트 추가.
...
- **GCP 다운로드 스크립트 파이프라인 연동**:
  ...
  - `download_from_gcs.sh` 다운로드 기본 경로를 NAS 경로로 변경 (`/nas/datasets/projects/source-a-data`).
