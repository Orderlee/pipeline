...
- **MVP 재개 v2 (Dual Path + Strict 8) 구현 완료**:
  - `docker/docker-compose.yaml`, `docker/.env` 정렬:
    ...
    - `/nas/datasets/projects` bind mount 추가(`PROJECTS_HOST_PATH=nas_secondary/datasets/projects`)
...
- **추가 작업 (오후 세션, 경로/실행 정렬)**:
  - `docker/docker-compose.yaml` 바인드 마운트 fallback 정렬:
    - `PROJECTS_HOST_PATH` fallback을 `nas_secondary/datasets/projects`로 통일
    - `INCOMING_HOST_PATH` fallback은 `/mnt/nas/incoming` 유지 (fallback 전용)
  - `docker/.env` 최종값 확인:
    - `PROJECTS_HOST_PATH=nas_secondary/datasets/projects`
...
- **Local/Docker 실행 동등성(Parity) 정리**:
  ...
  - `configs/global.yaml`:
    - `nas_root`: `nas_secondary/datasets/projects`
