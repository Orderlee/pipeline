...
사용법:
    ...
    python3 staging_test_dispatch.py --folder source-d --round 1
...
# ── 경로 설정 ──
STAGING_ROOT = Path("/home/user/mou/staging")
SOURCE_FOLDERS = {
    "tmp_data_2": STAGING_ROOT / "tmp_data_2",
    "source-d": STAGING_ROOT / "source-d",
}
...
def run_cycle(round_num: int) -> list[dict]:
    """한 사이클 (tmp_data_2 + source-d) 실행."""
    results = []

    for idx, folder in enumerate(["tmp_data_2", "source-d"], start=1):
...
def main():
    ...
    parser.add_argument("--folder", choices=["tmp_data_2", "source-d"], help="테스트할 폴더")
