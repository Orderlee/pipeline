...
# 사용: TEST_ROOT=/home/user/mou/staging bash scripts/test_qa_safe_drop_incoming.sh tmp_data_2
set -euo pipefail
TEST_ROOT="${TEST_ROOT:-${STAGING_ROOT:-/home/user/mou/staging}}"
SRC_NAME="${1:?folder name under $TEST_ROOT e.g. tmp_data_2}"
# QA_DROP_SOURCE: 대용량 폴더 대신 동일 폴더명의 소형 스모크 디렉터리 절대경로
SRC="${QA_DROP_SOURCE:-$TEST_ROOT/$SRC_NAME}"
BUILD="$TEST_ROOT/_qa_incoming_build"
if [[ ! -d "$SRC" ]]; then
  echo "소스 없음: $SRC" >&2
  exit 1
fi
...
rm -rf "$TEST_ROOT/incoming/$SRC_NAME"
mv "$BUILD/$SRC_NAME" "$TEST_ROOT/incoming/"
