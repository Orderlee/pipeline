...
# 컨테이너를 호스트 user UID(1000:1000)로 전환하는 1회성 migration.
#
# 실행 전:
...
TARGET_ROOT="${TARGET_ROOT:-/home/user/vlm-pipeline}"
...
echo "  target UID:GID    : $APP_UID:$APP_GID"
...
echo "[2/5] 기존 runtime 파일 소유권 복구 (user:user)"
sudo chown -R "$APP_UID:$APP_GID" "$SRC_DAGSTER_HOME" "$SRC_DATA" 2>/dev/null || true
...
sudo chown -R "$APP_UID:$APP_GID" "$DST_DAGSTER_HOME" "$DST_DATA"

echo "[5/5] 완료. 다음 단계:"
cat <<EOF

  1) docker/.env 또는 docker/.env.test 에서 아래 주석 해제:
       DOCKER_USER=${APP_UID}:${APP_GID}
