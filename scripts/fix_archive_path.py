"""source-c-event-bucket/20260402__2 -> 20260402 archive_path 수정."""
import duckdb
import sys
import time
...
for attempt in range(MAX_RETRIES):
    try:
        ...
        rows = conn.execute("""
            ...
            FROM raw_files
            WHERE archive_path LIKE '%source-c-event-bucket/20260402__2/%'
        """).fetchall()
        conn.close()
...
for attempt in range(MAX_RETRIES):
    try:
        ...
        updated = conn.execute("""
            UPDATE raw_files
            SET archive_path = REPLACE(archive_path, 'source-c-event-bucket/20260402__2/', 'source-c-event-bucket/20260402/')
            WHERE archive_path LIKE '%source-c-event-bucket/20260402__2/%'
        """).fetchone()
        conn.close()
...
for attempt in range(MAX_RETRIES):
    try:
        ...
        total = conn.execute("""
            SELECT COUNT(*) FROM raw_files
            WHERE archive_path LIKE '%source-c-event-bucket/20260402/%'
              AND source_unit_name = 'gcp/source-c-event-bucket/20260402'
        """).fetchone()
        conn.close()
        print(f"\nVerification:")
