#!/usr/bin/env python3
"""dispatch 트리거 이후 test 파이프라인 완료까지 DuckDB 읽기 전용 폴링."""
import argparse
import json
import os
import subprocess
...
COMPOSE_ENV_FILE = "docker/.env.test"
COMPOSE_PROJECT_NAME = os.getenv("TEST_COMPOSE_PROJECT_NAME", "pipeline-test")
TEST_DAGSTER_SERVICE = os.getenv("TEST_DAGSTER_SERVICE", "dagster-code-server")
TEST_DUCKDB_PATH = os.getenv("TEST_DUCKDB_PATH", "/data/staging.duckdb")
TEST_ROOT = os.getenv("TEST_ROOT") or os.getenv("STAGING_ROOT") or "/home/user/mou/staging"


def run_sql(query: str) -> int:
    code = f"""import duckdb
c = duckdb.connect({json.dumps(TEST_DUCKDB_PATH)}, read_only=True)
...
def main() -> None:
    ...
    while time.time() - start < args.max_wait:
        if args.mode == "timestamp":
            ...
            pth = Path(TEST_ROOT) / "incoming" / ".dispatch" / "failed" / f"{args.request_id}.json"
            if pth.is_file():
