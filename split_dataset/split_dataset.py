...
3) Example for a layout where JSONs live in a bboxes/ subfolder:
   - Images: image-data/cohort-a/processed_data/bat_as_weapon/20250812_AM_IS_CD_BW_1
   - JSONs : image-data/cohort-a/processed_data/bat_as_weapon/20250812_AM_IS_CD_BW_1/bboxes
   python split_dataset.py \
     --config configs/global.yaml \
     --source-bucket image-data \
     --json-prefix cohort-a/processed_data/bat_as_weapon/20250812_AM_IS_CD_BW_1/bboxes \
     --images-prefix cohort-a/processed_data/bat_as_weapon/20250812_AM_IS_CD_BW_1 \
     --assets-dir-prefix "" \
     --dest-bucket data-pipeline \
...
4) Batch mode: iterate category folders under a root prefix and merge all JSONs per category
   - Images: image-data/cohort-a/processed_data/bat_as_weapon/<folder>
   - JSONs : image-data/cohort-a/processed_data/bat_as_weapon/<folder>/bboxes
   python split_dataset.py \
     --config configs/global.yaml \
     --source-bucket image-data \
     --batch-root-prefix cohort-a/processed_data/bat_as_weapon \
...
5) Batch mode + merge multiple categories into one dataset (e.g. *_as_weapon -> weapon):
   python split_dataset.py \
     --config configs/global.yaml \
     --source-bucket image-data \
     --batch-root-prefix cohort-a/processed_data \
     --bboxes-subdir bboxes \
     --assets-dir-prefix "" \
     --dest-bucket data-pipeline \
