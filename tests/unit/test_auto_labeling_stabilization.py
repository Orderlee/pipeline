...
def _insert_raw_video(
    ...
    *,
    ...
    raw_key: str = "source-b-202512/20251222/sample.mp4",
) -> str:
...
def test_insert_label_single_row_succeeds(tmp_path: Path) -> None:
    ...
    resource.insert_label(
        {
            "label_id": "label-1",
            "asset_id": asset_id,
            "labels_key": "source-b-202512/20251222/sample.json",
    ...
    assert row == (
        "label-1",
        asset_id,
        "source-b-202512/20251222/sample.json",
        0.0,
        2.0,
    )
...
def test_replace_gemini_labels_rolls_back_on_insert_failure(tmp_path: Path) -> None:
    ...
    with resource.connect() as conn:
        rows = conn.execute(
            ...
            ORDER BY label_id
            """,
            [asset_id, "source-b-202512/20251222/sample.json"],
        ).fetchall()
...
def test_find_captioning_pending_videos_only_returns_generated_without_labels(tmp_path: Path) -> None:
    ...
    resource.update_auto_label_status(
        asset_id,
        "generated",
        label_key="source-b-202512/20251222/sample.json",
    ...
    resource.insert_processed_clip(
        {
            ...
            "clip_start_sec": 0.0,
            "clip_end_sec": 2.0,
            "clip_key": "source-b-202512/20251222/clips/sample_e000.mp4",
            "label_key": "source-b-202512/20251222/sample.json",
...
def test_clip_to_frame_mvp_extracts_buffered_window_but_stores_original_event_range(
    ...
    resource.insert_label(
        {
            ...
            "asset_id": asset_id,
            "labels_key": "source-b-202512/20251222/events/sample.json",
            "label_format": "gemini_event_json",
            "label_tool": "gemini",
            "label_source": "auto",
            "event_index": 0,
    ...
    assert row == (
        10.0,
        20.0,
        "source-b-202512/20251222/clips/sample_00010000_00020000.mp4",
    )
...
def test_clip_to_frame_routed_impl_extracts_buffered_window_but_stores_original_event_range(
    ...
    context = _DummyProcessContext(
        tags={"requested_outputs": "captioning", "folder_name": "source-b-202512"},
        op_config={"limit": 10},
    )
    ...
    assert row == (
        4.0,
        7.0,
        "source-b-202512/20251222/clips/sample_00004000_00007000.mp4",
    )
...
def test_find_ready_for_labeling_caption_backlog_returns_timestamp_completed_rows(tmp_path: Path) -> None:
    ...
    resource.update_timestamp_status(
        asset_id,
        "completed",
        label_key="source-b-202512/20251222/events/sample.json",
    ...
    assert rows == [
        {
            "asset_id": asset_id,
            "raw_bucket": "vlm-raw",
            "raw_key": "source-b-202512/20251222/sample.mp4",
            "timestamp_label_key": "source-b-202512/20251222/events/sample.json",
            "duration_sec": 12.5,
        }
    ]
...
def test_replace_processed_clip_frame_metadata_persists_image_caption_text(tmp_path: Path) -> None:
        ...
        [
            {
                ...
                "image_key": "source-b-202512/20251222/image/sample_00000001.jpg",
                ...
                "image_caption_key": "source-b-202512/20251222/image_captions/sample_00000001.json",
    ...
    assert rows[0]["image_caption_key"] == "source-b-202512/20251222/image_captions/sample_00000001.json"
...
def test_processed_clip_image_key_uses_sibling_image_directory() -> None:
    ...
    image_key = _build_processed_clip_image_key(
        "source-b-202512/20251222/clips/sample_e001_00000000_00002000.mp4",
        1,
    )

    assert image_key == "source-b-202512/20251222/image/sample_e001_00000000_00002000_00000001.jpg"
...
def test_image_caption_key_uses_sibling_image_captions_directory() -> None:
    ...
    caption_key = _build_image_caption_key(
        "source-b-202512/20251222/image/sample_e001_00000000_00002000_00000001.jpg"
    )

    assert caption_key == "source-b-202512/20251222/image_captions/sample_e001_00000000_00002000_00000001.json"
