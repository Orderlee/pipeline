...
class OrganizationMember(OrganizationMemberMixin, models.Model):
    ...
    role = models.CharField(
        ...
        default=ROLE_LABELER,
        help_text='Role of the member within the organization',
    )
    ...
    def get_user_role(cls, user, organization):
        """Return the membership role of ``user`` within ``organization``, or None if not a member."""
        return (
            cls.objects.filter(user=user, organization=organization).values_list('role', flat=True).first()
        )

    @classmethod
    def is_labeler(cls, user, organization):
        """Whether ``user`` participates in ``organization`` with the labeler role."""
        if user is None or not getattr(user, 'is_authenticated', False):
            return False
        if user.is_superuser:
            return False
        created_by_id = getattr(organization, 'created_by_id', None)
        if created_by_id is not None and created_by_id == user.id:
            return False
        return cls.get_user_role(user, organization) == cls.ROLE_LABELER

    @classmethod
    def is_admin(cls, user, organization):
        """Whether ``user`` should be treated as an admin within ``organization``.

        Superusers and the org owner (created_by) are treated as admins by convention;
        otherwise the membership role must be ``admin``.
        """
        if user is None or not getattr(user, 'is_authenticated', False):
            return False
        if user.is_superuser:
        ...
        if created_by_id is not None and created_by_id == user.id:
            return True
        return cls.get_user_role(user, organization) == cls.ROLE_ADMIN

    @classmethod
    def is_reviewer(cls, user, organization):
        """Whether ``user`` participates in ``organization`` with the reviewer role."""
        return cls.get_user_role(user, organization) == cls.ROLE_REVIEWER

    @classmethod
    def scope_tasks_for_user(cls, user, organization, queryset):
        """Narrow a task ``queryset`` to what ``user`` may see given their role in ``organization``.

        Labelers are restricted to tasks assigned to them; all other roles see the
        queryset unchanged. Kept role-agnostic so other task endpoints can reuse it.
        """
        if cls.is_labeler(user, organization):
            return queryset.filter(assignee=user)
        return queryset

    @classmethod
    def scope_projects_for_user(cls, user, organization, queryset):
        """Narrow a project ``queryset`` to what ``user`` may see given their role in ``organization``.

        Labelers are restricted to projects containing tasks assigned to them; all other
        roles see the queryset unchanged. Mirrors ``scope_tasks_for_user`` and reuses the
        same ``is_labeler`` exceptions (superuser/owner) for consistent behaviour.
        """
        if cls.is_labeler(user, organization):
    ...
    @classmethod
    def scope_tasks_for_request(cls, request, queryset):
        """Request-based convenience wrapper around ``scope_tasks_for_user``.

        Safely pulls the acting user and their active organization off ``request``
        and delegates. When request/user/organization is missing the queryset is
        returned unchanged, matching the safe default each caller relied on.
        """
        user = getattr(request, 'user', None)
        organization = getattr(user, 'active_organization', None)
        if user is None or organization is None:
            return queryset
        return cls.scope_tasks_for_user(user, organization, queryset)
