...
urlpatterns = [
    ...
    # admin 전용 현황 대시보드 SPA 진입점(포크 전용) — 권한은 API(is_admin)와 미들웨어가 강제.
    path('dashboard/', views.simple_view, name='dashboard-simple'),
    ...
    # admin 전용 현황 대시보드 집계(포크 전용). LABELER_ALLOWED 미등록 → 라벨러 자동 403.
    path('api/dashboard/summary', api.DashboardSummaryAPI.as_view(), name='dashboard-summary'),
    # invite
