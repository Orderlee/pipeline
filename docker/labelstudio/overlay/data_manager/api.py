        ...
        data.update(
            {
                ...
                'task_count': OrganizationMember.scope_tasks_for_request(request, project.tasks).count(),
                'annotation_count': Annotation.objects.filter(
                    task__in=OrganizationMember.scope_tasks_for_request(request, project.tasks.all())
                ).count(),
