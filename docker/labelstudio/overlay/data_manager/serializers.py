...
@extend_schema_field(
    {
        'type': 'array',
        'title': 'Assignee',
        'description': 'Labeler this task is assigned to',
        'items': {'type': 'object', 'title': 'User id', 'properties': {'user_id': {'type': 'integer'}}},
    }
)
class AssigneeDMFieldSerializer(serializers.SerializerMethodField):
    # GCP 외주 포크: assignee(담당 라벨러)를 updated_by 동형으로 노출 → 프론트가 멤버 이름 렌더.
    pass
...
class DataManagerTaskSerializer(TaskSerializer):
    ...
    # GCP 외주 포크: 태스크별 순수 활성 작업시간(admin 전용 DM 컬럼). annotate 후 직렬화 노출.
    avg_active_seconds = serializers.FloatField(required=False)
    draft_exists = serializers.BooleanField(required=False)
    updated_by = UpdatedByDMFieldSerializer(required=False, read_only=True)
    # GCP 외주 포크: 담당 라벨러(assignee) DM 컬럼 노출(admin/reviewer). updated_by 동형.
    assignee = AssigneeDMFieldSerializer(required=False, read_only=True)
    ...
    def get_assignee(obj):
        return [{'user_id': obj.assignee_id}] if obj.assignee_id else []
