...
def get_all_columns(project, user=None, *_):
    """Make columns info for the frontend data manager.

    ``user`` is passed by ProjectColumnsAPI (GET_ALL_COLUMNS(project, request.user)).
    GCP 외주 포크: admin 에게만 태스크별 순수 활성 작업시간(Active Time) 컬럼을 노출한다.
    """
    result = {'columns': []}
    ...
    result['columns'] += [
        ...
        },
        {
            ...
            'type': 'String',
            'target': 'tasks',
            'help': 'Task review status: pending / approved / rejected',
            'visibility_defaults': {'explore': True, 'labeling': True},
            'project_defined': False,
        },
        {
    ...
    # GCP 외주 포크: 태스크별 순수 활성 작업시간(Active Time) — admin 전용(정산/숙련도).
    # Lead Time(체류) 컬럼 바로 옆에 배치. 비-admin/익명은 컬럼 목록에서 제외(UI 게이팅).
    from organizations.models import OrganizationMember

    if OrganizationMember.is_admin(user, getattr(project, 'organization', None)):
        active_col = {
            'id': 'avg_active_seconds',
            'title': 'Active Time',
            'type': 'Time',
            'help': 'Average active (mouse/keyboard) work time over annotations; excludes idle gaps of 60s or longer. Admin only.',
            'target': 'tasks',
            'visibility_defaults': {'explore': True, 'labeling': False},
            'project_defined': False,
        }
        lead_idx = next(
            (i for i, c in enumerate(result['columns']) if c.get('id') == 'avg_lead_time'),
            len(result['columns']) - 1,
        )
        result['columns'].insert(lead_idx + 1, active_col)

    # GCP 외주 포크: 태스크가 어느 라벨러에게 배정됐는지(assignee) 컬럼 — admin·reviewer·owner 노출.
    # labeler 는 본인 배정분만 보므로 무의미 → 제외. updated_by 동형(List + project_members schema).
    if user is not None and not OrganizationMember.is_labeler(user, getattr(project, 'organization', None)):
        assignee_col = {
            'id': 'assignee',
            'title': 'Assignee',
            'type': 'List',
            'help': 'Labeler this task is assigned to. Admin/reviewer only.',
            'target': 'tasks',
            **({'schema': {'items': project_members}} if not remove_members_schema else {}),
            'visibility_defaults': {'explore': True, 'labeling': False},
            'project_defined': False,
        ...
        updated_by_idx = next(
            (i for i, c in enumerate(result['columns']) if c.get('id') == 'updated_by'),
            len(result['columns']) - 1,
        )
        result['columns'].insert(updated_by_idx + 1, assignee_col)
