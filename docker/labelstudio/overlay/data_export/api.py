...
from core.api_permissions import AdminOnly, HasObjectPermission
...
# Shared permissions for export endpoints: object-level access, authenticated,
# and labelers explicitly denied (labelers must not export project data).
# 데이터 반출 표면은 admin 전용(최소권한) — reviewer 도 403. 미디어 서빙 뷰는 대상 아님.
EXPORT_PERMISSION_CLASSES = [HasObjectPermission, IsAuthenticated, AdminOnly]
...
class ExportFormatsListAPI(generics.RetrieveAPIView):
    ...
    permission_classes = EXPORT_PERMISSION_CLASSES
