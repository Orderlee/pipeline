...
def bulk_update_is_labeled_by_overlap(tasks_ids, project):
    ...
    for i in range(0, len(tasks_ids), batch_size):
        ...
        # Rejected tasks must stay is_labeled=False (keep them in the labeler queue),
        # even if their annotation count meets the overlap requirement.
        Task.objects.filter(id__in=finished_task_ids, project=project).exclude(review_status=Task.ReviewStatus.REJECTED).update(
            is_labeled=True
        )
        Task.objects.filter(id__in=batch_ids, project=project).exclude(id__in=finished_task_ids).update(
            is_labeled=False
        )
        Task.objects.filter(id__in=batch_ids, project=project, review_status=Task.ReviewStatus.REJECTED).update(is_labeled=False)
