...
class Task(TaskMixin, FsmHistoryStateModel):
    ...
    assignee = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text='User this task is exclusively assigned to (labeler)',
    )

    class ReviewStatus(models.TextChoices):
        PENDING = 'pending', _('Pending')
        FIRST_APPROVED = 'first_approved', _('1차 승인')
    ...
    review_status = models.CharField(
        ...
        help_text='Task review status: pending / approved / rejected',
    )
    reviewed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text='Reviewer who last approved/rejected this task',
    )
    reviewed_at = models.DateTimeField(
        _('reviewed at'), null=True, blank=True, default=None, help_text='When the task was last reviewed'
    )
    review_note = models.TextField(
        _('review note'), null=True, blank=True, default='', help_text='Reviewer note, e.g. rejection reason'
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            # GCP 외주 포크: 검수 큐(DM Review 컬럼) 필터 — (project, review_status) 복합 인덱스.
            models.Index(fields=['project', 'review_status'], name='task_proj_review_idx'),
        ]
...
class Annotation(AnnotationMixin, FsmHistoryStateModel):
    ...
    # GCP 외주 포크: 순수 활성 작업시간(초). 프론트가 마우스/키보드 활동 기반으로 측정하며
    # 60초 이상 무활동 공백은 제외한다(lead_time = 체류시간, active_seconds = 실작업시간).
    active_seconds = models.FloatField(
        _('active seconds'),
        null=True,
        default=None,
        help_text='Active (mouse/keyboard) work time in seconds; excludes idle gaps of 60s or longer',
    )
...
class TaskReviewHistory(models.Model):
    """Audit log of per-task review transitions (GCP outsourced LS fork, self-contained F1)."""

    class Action(models.TextChoices):
        FIRST_APPROVED = 'first_approved', _('First approved')
        APPROVED = 'approved', _('Approved')
        REJECTED = 'rejected', _('Rejected')
        REOPENED = 'reopened', _('Reopened')

    class RejectReason(models.TextChoices):
        """반려 사유 코드(Error Class) — 사유별 통계·외주 가이드 재교육용.

        도입(2026-07-10) 이전 반려 이력은 코드가 없어 '미분류(null)'로 집계된다.
        도메인 확장 시 항목 추가는 허용, 기존 코드 제거/의미 변경은 통계 연속성 때문에 금지.
        """

        FALSE_POSITIVE = 'fp', _('오탐(없는 이벤트 라벨) FP')
        FALSE_NEGATIVE = 'fn', _('미탐(이벤트 누락) FN')
        BAD_BOUNDARY = 'boundary', _('경계·박스 불량')
        WRONG_CLASS = 'wrong_class', _('클래스 오류')
        BAD_TIMESTAMP = 'timestamp', _('타임스탬프 오차')
        GUIDELINE = 'guideline', _('지침 위반')
        OTHER = 'other', _('기타')

    task = models.ForeignKey(
        Task, on_delete=models.CASCADE, related_name='review_history', help_text='Task ID'
    )
    actor = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='task_review_history',
        help_text='Actor User ID',
    )
    action = models.CharField(max_length=32, choices=Action.choices, help_text='Review transition action')
    note = models.TextField(null=True, blank=True, default='', help_text='Reviewer note, e.g. rejection reason')
    reason_code = models.CharField(
        ...
        blank=True,
        default=None,
        help_text='Structured rejection reason (Error Class); null for approvals and pre-adoption history',
    )
...
def update_project_summary_annotations_and_is_labeled(sender, instance, created, **kwargs):
    ...
    # When a previously rejected task gets (re)labeled, send it back to the review queue as pending.
    # review_status is re-read from the DB (single targeted column read) rather than trusting
    # instance.task.review_status: the review API persists review_status on its own Task instance
    # via update_fields, so the cached instance reaching this signal can hold a stale value and
    # would otherwise miss the rejected->pending reopen. Do not replace this with the in-memory field.
    if instance.task.is_labeled:
        db_review_status = (
            Task.objects.filter(pk=instance.task.pk).values_list('review_status', flat=True).first()
        )
        if db_review_status == Task.ReviewStatus.REJECTED:
            instance.task.review_status = Task.ReviewStatus.PENDING
            update_fields.append('review_status')
            reopen_actor = instance.completed_by or instance.task.assignee
            if reopen_actor is not None:
                TaskReviewHistory.objects.create(
                    task=instance.task, actor=reopen_actor, action=TaskReviewHistory.Action.REOPENED
                )
    instance.task.save(update_fields=update_fields, skip_fsm=True)
