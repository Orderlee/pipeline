...
from tasks.models import Annotation, AnnotationDraft, Prediction, Task, TaskReviewHistory
...
class TaskListAPI(DMTaskListAPI):
    ...
    def get_task_queryset(self, request, prepare_params):
        queryset = super().get_task_queryset(request, prepare_params)
        from organizations.models import OrganizationMember

        return OrganizationMember.scope_tasks_for_request(request, queryset)
...
class TaskAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        lean_queryset = Task.objects.filter(
            project__organization=self.request.user.active_organization
        ).select_related('project')
        lean_queryset = OrganizationMember.scope_tasks_for_request(self.request, lean_queryset)
        lean_task = generics.get_object_or_404(lean_queryset, pk=task_id)
...
class TaskReviewAPI(generics.GenericAPIView):
    """Self-contained per-task review (GCP outsourced LS fork).

    Reviewer/admin approves or rejects a labeler's task result. Rejected tasks
    return to the labeler queue (is_labeled=False). Labelers are blocked (403).
    """
    ...
    def post(self, request, pk):
        task = (
            Task.objects.filter(pk=pk, project__organization=request.user.active_organization).first()
        )
        if task is None:
        ...
        # Per-action state transition: each branch sets only the fields it owns
        # and declares which columns to persist + which audit action to log.
        # The common bits (reviewer/timestamp, save, history entry) run once below.
        if action == 'approve':
            from organizations.models import OrganizationMember

            # 승인은 라벨(제출)된 태스크에만. rejected(is_labeled=False)·미착수는 재라벨 후 대상.
            if not task.is_labeled:
                raise ValidationError('task must be labeled (submitted) before it can be approved')

            org = request.user.active_organization
            # Two-stage sequential review: reviewer approval is only a first pass
            # (first_approved); admin approval is the final approval (approved).
            if OrganizationMember.is_admin(request.user, org):
                task.review_status = Task.ReviewStatus.APPROVED
                history_action = TaskReviewHistory.Action.APPROVED
            else:
                # 강등 금지: 이미 최종 승인(approved)된 태스크를 reviewer 가 되돌리지 못하게(데이터 보호).
                if task.review_status == Task.ReviewStatus.APPROVED:
                    raise ValidationError('task is already finally approved; a reviewer cannot downgrade it')
                # 재승인 금지: 이미 1차 통과(first_approved)한 태스크는 최종검수(admin) 대기 상태이므로
                # reviewer 가 다시 승인하여 reviewed_by/reviewed_at 을 덮지 못하게(최종검수 큐 보호).
                if task.review_status == Task.ReviewStatus.FIRST_APPROVED:
                    raise ValidationError('task already passed 1st review; awaiting final approval')
                task.review_status = Task.ReviewStatus.FIRST_APPROVED
                history_action = TaskReviewHistory.Action.FIRST_APPROVED
            ...
            if not note.strip():
                raise ValidationError('note is required when rejecting a task')
            # 사유 코드(Error Class, 선택) — 통계용. 미지정(구 클라이언트)은 null=미분류로 집계.
            ...
            if reason_code is not None and reason_code not in TaskReviewHistory.RejectReason.values:
                raise ValidationError(
                    f'invalid reason; must be one of {list(TaskReviewHistory.RejectReason.values)}'
                )
            task.review_status = Task.ReviewStatus.REJECTED
            task.is_labeled = False
            task.review_note = note
            update_fields = ['review_status', 'is_labeled', 'review_note', 'reviewed_by', 'reviewed_at']
            history_action = TaskReviewHistory.Action.REJECTED
        else:
        ...
        TaskReviewHistory.objects.create(
            ...
            note=note,
            reason_code=reason_code if action == 'reject' else None,
        )
...
class AnnotationAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        scoped_tasks = OrganizationMember.scope_tasks_for_request(
            self.request, Task.objects.for_user(self.request.user)
        )
        return Annotation.objects.filter(task__in=scoped_tasks)
...
class AnnotationsListAPI(GetParentObjectMixin, generics.ListCreateAPIView):
    ...
    def _get_parent_object(self):
        ...
        self.parent_queryset = OrganizationMember.scope_tasks_for_request(
            self.request, Task.objects.for_user(self.request.user)
        )
        return super()._get_parent_object()
    ...
    def get_queryset(self):
        ...
        task = generics.get_object_or_404(scoped_tasks, pk=self.kwargs.get('pk', 0))
...
class AnnotationDraftListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        scoped_tasks = OrganizationMember.scope_tasks_for_request(self.request, Task.objects.for_user(self.request.user))
        return AnnotationDraft.objects.filter(task__in=scoped_tasks)
    ...
    def perform_create(self, serializer):
        ...
        user = self.request.user
        scoped_tasks = OrganizationMember.scope_tasks_for_request(
            self.request, Task.objects.for_user(user)
        )
        generics.get_object_or_404(scoped_tasks, pk=task_id)
...
class PredictionAPI(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        active_org = self.request.user.active_organization
        scoped_tasks = OrganizationMember.scope_tasks_for_request(
            self.request, Task.objects.filter(project__organization=active_org)
        )
        return Prediction.objects.filter(task__in=scoped_tasks)
