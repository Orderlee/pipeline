...
from projects.models import LabelStateHistory, Project, ProjectImport, ProjectManager, ProjectReimport, ProjectSummary
...
class ProjectListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        projects = OrganizationMember.scope_projects_for_user(
            self.request.user, self.request.user.active_organization, projects
        )
...
class ProjectNextTaskAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        prepared_tasks = OrganizationMember.scope_tasks_for_request(request, prepared_tasks)
...
@extend_schema(exclude=True)
class ProjectSubmitAPI(generics.GenericAPIView):
    """Self-contained F1 submit / re-submit.

    Marks the project as submitted (idempotent soft re-submit) and appends an
    audit row to label_state_history. No pipeline / Dagster / external PG calls.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        # stage: 'mid'(중간 검수 완료=최종 검수 대기, reviewer/admin) / 'final'(최종 검수 완료=라벨링 완료, admin).
        # 기본 'final'(구 클라이언트 호환). 게이팅: mid=pending 0 / final=전부 approved + 중간 완료 선행.
        stage = (request.data.get('stage') or 'final').lower()
        tasks = Task.objects.filter(project=project)
        total = tasks.count()
        pending = tasks.filter(review_status=Task.ReviewStatus.PENDING).count()
        approved = tasks.filter(review_status=Task.ReviewStatus.APPROVED).count()

        if stage == 'mid':
            if total == 0 or pending != 0:
                raise RestValidationError(
                    f'중간 검수 완료 불가: 1차 검수 대기(pending) 태스크가 {pending}건 남았습니다.'
                )
            already = bool(project.is_mid_reviewed)
            if not already:
                project.is_mid_reviewed = True
                project.mid_reviewed_at = timezone.now()
                project.save(update_fields=['is_mid_reviewed', 'mid_reviewed_at'])
            LabelStateHistory.objects.create(
                project=project, actor=request.user, action=LabelStateHistory.Action.MID_SUBMITTED
            )
            return Response(
                {
                    'stage': 'mid',
                    'is_mid_reviewed': True,
                    'mid_reviewed_at': project.mid_reviewed_at,
                    'is_resubmit': already,
        ...
        # stage == 'final' — admin 전용, 중간 완료 선행 + 전부 approved 필요.
        if not OrganizationMember.is_admin(request.user, project.organization):
            raise PermissionDenied('최종 검수 완료는 admin 만 가능합니다.')
        if not project.is_mid_reviewed:
            raise RestValidationError('최종 검수 완료 불가: 먼저 중간 검수 완료가 필요합니다.')
        if total == 0 or approved != total:
            raise RestValidationError(
                f'최종 검수 완료 불가: 모든 태스크가 approved 여야 합니다(현재 {approved}/{total}).'
            )

        is_resubmit = bool(project.is_submitted)
        if not is_resubmit:
            project.is_submitted = True
            project.submitted_at = timezone.now()
            project.save(update_fields=['is_submitted', 'submitted_at'])

        action = LabelStateHistory.Action.RESUBMITTED if is_resubmit else LabelStateHistory.Action.SUBMITTED
        LabelStateHistory.objects.create(project=project, actor=request.user, action=action)
...
@extend_schema(exclude=True)
class ProjectRejectAPI(generics.GenericAPIView):
    """Self-contained F1 reject — admin-only "one stage back" project revert.

    프로젝트 완료 단계를 **정확히 한 단계** 되돌린다(admin 전용). 태스크 review_status 는
    보존한다(개별 불량 태스크 반려는 태스크 단위 Reject 로 별도 처리):
      - final(완료, is_submitted)          → mid(최종 검수 대기)  : is_submitted=False
      - mid(최종 검수 대기, is_mid_reviewed) → open(진행 중)       : is_mid_reviewed=False
    되돌릴 완료 단계가 없으면(open) 거부(허위 반려 이력 누적 방지).
    감사 이력(action='rejected', note=reason) 을 label_state_history 에 남긴다.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not OrganizationMember.is_admin(request.user, project.organization):
            raise PermissionDenied('Only admins can reject a project')

        note = request.data.get('note', '')
        if not isinstance(note, str) or not note.strip():
            raise RestValidationError('note is required and must be a non-empty string')

        # 한 단계 되돌림: final 이면 최종만 해제, 아니면 중간 해제. (태스크 상태는 불변)
        # 상태 변경 + 감사 이력은 원자적으로(중간 실패 시 미로깅 상태변경 방지).
        ...
        elif project.is_mid_reviewed:
            reverted = 'mid->open'
            update_fields = ['is_mid_reviewed', 'mid_reviewed_at']
            project.is_mid_reviewed = False
            project.mid_reviewed_at = None
        else:
            raise RestValidationError('project has no completed stage to revert (already open)')
        ...
        with transaction.atomic():
            ...
            LabelStateHistory.objects.create(
                project=project,
                actor=request.user,
                action=LabelStateHistory.Action.REJECTED,
                note=note,
            )

        # stage 는 되돌림 후 실제 플래그에서 파생(하드코딩 금지 — 비정상 입력 상태에서도 진실을 반환).
        new_stage = 'final' if project.is_submitted else ('mid' if project.is_mid_reviewed else 'open')
        ...
        return Response(
            {
                ...
                'is_mid_reviewed': project.is_mid_reviewed,
...
@extend_schema(exclude=True)
class ProjectSubmitStateAPI(generics.GenericAPIView):
    """Self-contained F1 submit-state read.

    Returns the current finalize state of the project so the front-end can
    render the submit button label/state. No pipeline / external PG calls.
    """

    # submit-state 는 읽기(is_submitted 조회)라 labeler 도 허용 — 단 접근 가능한 프로젝트로 스코프.
    # (프론트 SubmitButtonWrapper 가 마운트 시 fetch. 버튼 자체는 프론트에서 labeler 에게 숨김.)
    ...
    def get(self, request, *args, **kwargs):
        ...
        pk = self.kwargs.get('pk')
        base = Project.objects.filter(pk=pk, organization=request.user.active_organization)
        project = OrganizationMember.scope_projects_for_user(request.user, request.user.active_organization, base).first()
        if project is None:
            raise NotFound(f'Project {pk} not found')

        last = (
            LabelStateHistory.objects.filter(
                project=project,
                action__in=[LabelStateHistory.Action.SUBMITTED, LabelStateHistory.Action.RESUBMITTED],
            )
            .order_by('-created_at')
            .first()
        )
        submitted_by = last.actor_id if last else None

        # 2단계 검수 상태 + 게이팅 카운트(프론트 버튼 활성/라벨 결정).
        #   stage: open → mid(최종 검수 대기) → final(완료)
        #   can_mid  = 아직 중간 미완료 & pending 0 & total>0
        #   can_final = 중간 완료 & 아직 최종 미완료 & 전부 approved
        tasks = Task.objects.filter(project=project)
        total = tasks.count()
        pending = tasks.filter(review_status=Task.ReviewStatus.PENDING).count()
        approved = tasks.filter(review_status=Task.ReviewStatus.APPROVED).count()
        if project.is_submitted:
            ...
            stage = 'open'
        can_mid = (not project.is_mid_reviewed) and total > 0 and pending == 0
        can_final = project.is_mid_reviewed and (not project.is_submitted) and total > 0 and approved == total
        changed_task_count = (
            Task.objects.filter(project=project, updated_at__gt=project.submitted_at).count()
            if project.is_submitted and project.submitted_at
            else 0
        )
        return Response(
            {
                ...
                # 프론트(SubmitButtonWrapper) 계약 — 최종 완료 상태.
                'is_finalized': project.is_submitted,
                'last_finalized_at': project.submitted_at,
                'last_finalized_by': submitted_by,
