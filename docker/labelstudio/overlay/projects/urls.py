...
_api_urlpatterns = [
    ...
    # Self-contained F1 submit / re-submit (no trailing slash, matches spec)
    path('<int:pk>/submit', api.ProjectSubmitAPI.as_view(), name='project-submit'),
    # Self-contained F1 reject (admin-only revert of submit, no trailing slash)
    path('<int:pk>/reject', api.ProjectRejectAPI.as_view(), name='project-reject'),
    # Self-contained F1 submit-state read (no trailing slash, matches submit convention)
    path('<int:pk>/submit-state', api.ProjectSubmitStateAPI.as_view(), name='project-submit-state'),
]
