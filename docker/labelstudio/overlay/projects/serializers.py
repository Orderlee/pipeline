...
class ProjectSerializer(FlexFieldsModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        # For labelers, scope count fields to their own assigned tasks so that
        # project-wide totals from with_counts annotation do not leak. Other roles
        # (reviewer/admin) and internal calls without a request keep annotate values.
        ...
        if organization is None or not OrganizationMember.is_labeler(user, organization):
            return data

        scoped_tasks = OrganizationMember.scope_tasks_for_user(user, organization, instance.tasks)
        if 'task_number' in data:
            data['task_number'] = scoped_tasks.count()
        if 'total_annotations_number' in data:
            data['total_annotations_number'] = Annotation.objects.filter(task__in=scoped_tasks).count()
