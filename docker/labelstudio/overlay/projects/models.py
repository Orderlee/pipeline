...
class Project(ProjectMixin, FsmHistoryStateModel):
    ...
    is_submitted = models.BooleanField(
        _('is submitted'), default=False, help_text='Whether this project has been submitted (self-contained F1).'
    )
    submitted_at = models.DateTimeField(
        _('submitted at'), null=True, default=None, help_text='Timestamp of the first submit.'
    )
    # GCP 외주 포크: 프로젝트 2단계 검수. is_mid_reviewed(중간 검수 완료=최종 검수 대기, reviewer/admin)
    # → is_submitted(최종 검수 완료=라벨링 완료, admin). Reject Project 는 최종→중간대기 1단계 되돌림.
    is_mid_reviewed = models.BooleanField(
        _('is mid reviewed'), default=False, help_text='중간 검수 완료(최종 검수 대기). 1차 검수자 완료 표시.'
    )
    mid_reviewed_at = models.DateTimeField(
        _('mid reviewed at'), null=True, default=None, help_text='중간 검수 완료 시각.'
    )
...
class LabelStateHistory(models.Model):
    """Audit log of project submit/re-submit transitions (self-contained F1)."""

    class Action(models.TextChoices):
        SUBMITTED = 'submitted', _('Submitted')
        RESUBMITTED = 'resubmitted', _('Resubmitted')
        REJECTED = 'rejected', _('Rejected')
        MID_SUBMITTED = 'mid_submitted', _('Mid submitted')  # 중간 검수 완료(최종 검수 대기)

    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name='label_state_history', help_text='Project ID'
    )
    actor = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='label_state_history', help_text='Actor User ID'
    )
    action = models.CharField(max_length=32, choices=Action.choices, help_text='Transition action')
    note = models.TextField(null=True, blank=True, default='', help_text='Optional note (e.g. rejection reason)')
