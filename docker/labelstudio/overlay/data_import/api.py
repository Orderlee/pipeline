...
class ImportAPI(generics.CreateAPIView):
    ...
    # 데이터 투입은 admin 전용(최소권한) — 표준 경로는 import 스크립트(운영가이드 2.2)
    permission_classes = api_settings.DEFAULT_PERMISSION_CLASSES + [ProjectImportPermission, AdminOnly]
...
class ImportPredictionsAPI(generics.CreateAPIView):
    permission_classes = api_settings.DEFAULT_PERMISSION_CLASSES + [AdminOnly]
    """
...
class UploadedFileResponse(generics.RetrieveAPIView):
    ...
    def get(self, *args, **kwargs):
        ...
        if file_upload is None or not file_upload.has_permission(request.user):
            return Response(status=status.HTTP_403_FORBIDDEN)

        # labeler assignee scoping: only allow files linked to the labeler's assigned tasks
        active_org = getattr(request.user, 'active_organization', None)
        if active_org is not None and OrganizationMember.is_labeler(request.user, active_org):
            scoped_tasks = OrganizationMember.scope_tasks_for_request(request, Task.objects.for_user(request.user))
            if not file_upload.tasks.filter(id__in=scoped_tasks).exists():
