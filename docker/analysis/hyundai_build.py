#!/usr/bin/env python3
"""현대백화점 이벤트 구간 → SAM3.1 검출 → 임베딩 → FiftyOne `Hyundai` 데이터셋.

요청(2026-08-05): Hyundai_* 로 진행 / `/datasets/Hyundai` / **SAM3.1 로 이벤트 구간만 이미지
...
스테이지:
    ...
    build     FiftyOne `Hyundai` + UMAP + 필드
    all       위 전부 순차

이후 뱅크 분석은 prompt_geometry.py 를 `--profile hyundai` 로 재사용한다:
    BANK_A=v1.0.8.0 BANK_B=v1.0.8.0 python prompt_geometry.py attach --profile hyundai
...
ROOT = os.environ.get("HY_ROOT", "/data/fiftyone/hyundai")
WORK = f"{ROOT}/work"
FRAMES = f"{ROOT}/frames"
DATASET = os.environ.get("HY_DATASET", "Hyundai")
...
def stage_segments() -> None:
    rows = psql("""
        ...
        WHERE r.source_unit_name LIKE 'Hyundai%'
