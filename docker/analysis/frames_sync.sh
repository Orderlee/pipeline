...
#
#   ./docker/analysis/frames_sync.sh            # scan → copy → embed (증분)
#   ./docker/analysis/frames_sync.sh --build    # + score → build → report (클래스가 갖춰진 뒤)
#   COPY_LIMIT=1500 ./docker/analysis/frames_sync.sh   # 배치 크기 제한 (업로드 중 부하 완화)
#
# 왜 컨테이너를 두 개 쓰나:
