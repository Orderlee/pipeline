...
#   5. atlas   — 문장↔이미지↔영상 연결 2종 (prompt_frames / video_prompts CSV)
#   6. viz     — margin_viz(사분면) 산점도
...
#
# 판정 기준 (docs/prompt-geometry-2026-07-31.md §11):
...
#
# 전제: source-h 프레임 데이터셋이 준비돼 있어야 한다 (frames_sync.sh --build).
set -euo pipefail

VER_A="${1:?사용법: bank_eval.sh <기준버전> <신버전> [신버전CSV]}"
VER_B="${2:?신버전 이름 필요}"
CSV_B="${3:-}"
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
C="${ANALYSIS_CONTAINER:-docker-analysis-1}"

docker cp "$REPO/docker/analysis/prompt_geometry.py" "$C:/workspace/" >/dev/null

run() { docker exec -e BANK_A="$VER_A" -e BANK_B="$VER_B" "$C" \
        python3 /workspace/prompt_geometry.py "$@"; }
...
for st in analyze gap flips prune atlas viz guide slim report; do
  run "$st"
done
...
echo "  · 삭제 랭킹(전 뱅크): /data/fiftyone/sourceh_v2/report/prune_<version>.csv"
echo "  · 문장→영상/프레임: prompt_frames_<version>.csv | 영상→문장: video_prompts_<version>.csv"
