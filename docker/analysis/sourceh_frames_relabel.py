#!/usr/bin/env python3
"""source-h 프레임 **재라벨**(클래스 폴더 이동) — `sourceh_frames_eval.py` 산출물까지 함께 정합.
...
1. **`work/` 산출물이 `<folder>/<name>` 키다** — `embed.npz`, `angle_frames.jsonl`, `ledger.jsonl`,
   ...
   키만 옮기면 된다**.
2. **라이브 FiftyOne 데이터셋의 filepath 가 깨진다** — `source-h` 은 `sourceh_prompt_geometry.py` 가
...
사용 (analysis 컨테이너 안에서):
    docker cp docker/analysis/sourceh_frames_relabel.py docker-analysis-1:/tmp/
    docker exec docker-analysis-1 python /tmp/sourceh_frames_relabel.py normal \\
        폐기물보관장_연기_20260320_040557_0001 폐기물보관장_연기_20260320_040557_0002
    docker exec docker-analysis-1 python /workspace/sourceh_frames_eval.py score   # 점수 갱신

⚠️ **NAS 원본(`/home/user/mou/nas_primary/source-h/<class>/`)은 건드리지 않는다.** 즉
`sourceh_frames_eval.py scan` 을 다시 돌리면 옛 폴더로 되살아난다 — 영구 반영은 NAS 쪽 이동 필요.
...
def main() -> None:
    ...
    print("완료 — 이어서: python /workspace/sourceh_frames_eval.py score")
