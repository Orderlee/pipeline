...
기존 `sourceh_prompt_eval.py`(영상 단위, MinIO 소스)의 후속. 근본적으로 다른 데이터다:
...
사용:
    python3 /workspace/sourceh_frames_eval.py scan
    python3 /workspace/sourceh_frames_eval.py copy --limit 1500
    python3 /workspace/sourceh_frames_eval.py embed --limit 2000
    python3 /workspace/sourceh_frames_eval.py all --limit 2000     # 업로드 끝나면 --limit 없이
"""
...
#     datapipeline-analysis:latest python3 /ws/sourceh_frames_eval.py copy --limit 1500
...
def stage_score() -> None:
    ...
    for ver in VERSIONS:
        ...
        if not os.path.exists(p):
            raise SystemExit(f"프롬프트 npz 없음: {p} — sourceh_prompt_eval.py prompts 를 먼저 실행")
