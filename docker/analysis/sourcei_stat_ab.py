#!/usr/bin/env python3
"""배경통계 **모수 교체 + 클래스별 부호** A/B — §27 하네스를 그대로 쓴다.

§27-4 는 배경통계가 `frames`(22프로젝트) 위에서 계산되고 채점은 sourcei 에서 된다는 것을
지적했다. `sourcei_native_stats.py` 가 그 다음을 실측했다 (카메라 중심화 판별력과의 spearman):

    클래스     sd_frames   sd_sourcei
    normal      +0.014      -0.717      ← 부호가 반대
    falldown    +0.052      +0.033      ← 신호 없음
    fire        +0.456      +0.674
    smoke       +0.182      +0.882
    전체        -0.415      +0.337      ← 현행은 **역방향**

여기서 그 두 가지를 실제 선택에 적용해 검정한다. **판정 설정·중복제거·혼합비는 §27 base 와
한 글자도 다르지 않게** 두고 통계 모수와 정렬 부호만 바꾼다 — base 가 §27 의 mF1 0.5482 를
재현하지 못하면 비교가 성립하지 않으므로 실행 중 검사한다.

⚠️ 부호는 **같은 데이터의 상관에서 왔다.** 여기 결과는 효과크기 추정이지 가설검정이 아니다.
   확정하려면 카메라 홀드아웃이 필요하다(보고 시 반드시 병기).
...
STATS_SRC = os.environ.get("AB_STATS", "frames")     # frames | sourcei
_np_ = {"frames": f"{OUT}/sent_stats_byhash.npz", "sourcei": f"{OUT}/sent_stats_sourcei.npz"}
_st = np.load(_np_["frames"], allow_pickle=True)
_st_hy = np.load(_np_["sourcei"], allow_pickle=True)
...
def _pick2(st_, pos_, name):
    p2 = {h: i for i, h in enumerate(st_["hashes"])}
    r2 = np.array([p2.get(h, -1) for h in hashes], np.int64); o2 = r2 >= 0
    a = np.full(len(hashes), np.nan, np.float32); a[o2] = st_[name][r2[o2]]
    return a
m_s_mean, m_s_max, sd_sup = _pick("m_s_mean"), _pick("m_s_max"), _pick("sd")
HY = {n: _pick2(_st_hy, None, n) for n in ("m_s_mean", "m_s_max", "sd")}
# 클래스별 정렬 부호 — sourcei_native_stats.py 실측. +1 = sd 상위가 좋다.
...
def build_pool(agg, polarity, stats="frames", signed=False):
    if stats == "sourcei":
        ms = HY["m_s_max"] if agg == "max" else HY["m_s_mean"]; sdv = HY["sd"]
    else:
        ms = m_s_max if agg == "max" else m_s_mean; sdv = sd_sup
    rows, sdk, drops = [], [], {}
    for c in CLASSES:
        ii = np.array(sup_idx[c])
        sg = SIGN[c] if signed else +1
        key = sg * sdv[ii]                       # 부호 적용 특이도 (정렬·컷의 유일한 축)
        unspec = key <= np.percentile(key, 25)
        noisy = ms[ii] >= np.percentile(ms[ii], 75)
        rm = (unspec & noisy) if polarity == "and" else unspec
        keep = ii[~rm]; drops[c] = int(rm.sum())
        q = (sg * sdv[keep]) * (1 - (ms[keep] - ms[keep].min()) / (np.ptp(ms[keep]) + 1e-9) * .5)
        keep = keep[np.argsort(-q)[:POOL_PER_CLS]]
        for j in keep: rows.append(("sup", int(j), stext.get(hashes[j], ""), c, "공급"))
        sdk.extend((sg * sdv[keep]).tolist())
    nsup = len(rows)
...
VARIANTS = [
    # (이름, agg, polarity, dedup, thr, stats, signed, 설명)
    ("base",      "mean", "or", "cos", CFG["dedup"], "frames",  False, "§27 base 재현 (검사용)"),
    ("hy",        "mean", "or", "cos", CFG["dedup"], "sourcei", False, "모수만 sourcei 로 교체"),
    ("hy_signed", "mean", "or", "cos", CFG["dedup"], "sourcei", True,  "모수 sourcei + normal 부호 반전"),
    ("fr_signed", "mean", "or", "cos", CFG["dedup"], "frames",  True,  "모수 그대로 + normal 부호 반전"),
]

# ══ 1) 변형별 예측 캐시 (결정론) ════════════════════════════════════
cache_pool, PRED, WIOU, SEL, DROPS, NSENT = {}, {}, {}, {}, {}, {}
for name, agg, pol, mode, thr, stats_, signed_, why in VARIANTS:
    kk = (agg, pol, stats_, signed_)
...
assert abs(POOLED["base"]["mf1"] - 0.5482) < 1e-4, \
    f"base 가 §27(0.5482) 을 재현하지 못한다: {POOLED['base']['mf1']} — 비교 무효"
json.dump(dict(cfg=CFG, n_boot=NBOOT, sign=SIGN, n_cameras=len(CAMS), n_cameras_macro=len(usable),
          ...
          open(f"{ABDIR}/hy_stat_ab.json", "w"), ensure_ascii=False, indent=1)
log(f"→ {ABDIR}/hy_stat_ab.json")
for n in names:
    r = RES[n]
