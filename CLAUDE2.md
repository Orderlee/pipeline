...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  [DONE] MinIO endpoint 콘솔/API 분리 정정 + 환경 통일 (2026-03-06)
    ...
    - 조치:
      - `docker/docker-compose.yaml`, `.env.example`, `docker/.env`
        - `MINIO_ENDPOINT=http://10.0.0.36:9000` 통일
  ...
  [DONE] INGEST MinIO 업로드 재활성화 + raw_key prefix 제거 (2026-03-06)
    ...
    - 검증:
      - 신규 sample key가 `source-a-rtsp-bucket/...`, `source-b-event-bucket/...` 형태로 생성됨
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  2) 증상: staging을 재테스트하려는데 이전 실행 결과 때문에 새 테스트가 섞여 보임
    ...
    - 권장 초기화 순서:
      ...
      - B. staging MinIO endpoint(`10.0.0.36:9002`)에서
        - `vlm-raw`
        - `vlm-labels`
        - `vlm-processed`
        - `vlm-dataset`
  ...
  5) 운영 참고: staging incoming 배치의 Gemini/clip/YOLO 비용과 시간을 다시 계산해야 할 때
    ...
    - 현재 문서 범위:
      - 대상: `/home/user/mou/staging/incoming` 당시 적재 영상 38개
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  3) 증상: 호스트의 `/home/user/mou/staging/incoming`에는 파일이 있는데 staging 컨테이너가 파일을 못 봄
    - 대표 현상:
      - 컨테이너 안 `/nas/staging/incoming`에 `.manifests`만 있고 실제 비디오는 없음.
      - `incoming_manifest_sensor`가 계속 `pending 없음`처럼 보일 수 있음.
    - 실제 원인:
      - 기본 공통 볼륨은 `/home/user/mou/incoming -> /nas/incoming`만 연결.
      - staging 전용 경로 `/home/user/mou/staging/incoming`, `/home/user/mou/staging/archive`가 별도 마운트되지 않음.
    - 영구 조치:
