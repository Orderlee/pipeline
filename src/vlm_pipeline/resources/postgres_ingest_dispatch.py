"""PG INGEST 도메인 — dispatch tracking 관련 메서드 (DuckDBIngestDispatchMixin 1:1 포팅).

dispatch_requests / dispatch_pipeline_runs DDL은 sql/migrations/postgres/001_init.sql 에서 생성된다
(CLAUDE.md "schema = 최종 상태" 원칙).
"""
...
class PostgresIngestDispatchMixin:
    ...
    def get_dispatch_request_status(self, request_id: str) -> str | None:
        ...
        with self.connect() as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT status FROM dispatch_requests WHERE request_id = %s",
        ...
        with self.connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'failed', %s, %s)
