...
def _spec_status_from_method(method: list[str]) -> str:
    if not method:
        return "pending"
    return "active"
...
def ingest_router(
    ...
    """source_unit_name 기준 spec 매칭, config resolve, raw_files.ingest_status 전이."""
    db.ensure_schema()
    ...
    for source_unit_name, rows in by_unit.items():
        if not source_unit_name or source_unit_name == "_unknown_":
            # source_unit_name 없으면 pending_spec
