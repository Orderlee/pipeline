"""Staging-only manual import path for already-labeled datasets."""
...
from vlm_pipeline.defs.label.artifact_import_support import import_local_label_artifacts
...
def _scan_artifact_json_paths(
    *,
    ...
    if scan_local_dirs:
        ordered_roots.extend(_scan_local_artifact_dirs(source_unit_dir, dir_names))
    if scan_global_dirs:
...
def _find_raw_asset_for_media(db: DuckDBResource, source_unit_name: str, *, image_stem: str) -> dict[str, Any] | None:
...
def _ensure_processed_media_rows(
    ...
    *,
    ...
    image_path = _resolve_local_file(
        source_unit_dir,
    ...
    image_key = _normalize_unit_scoped_key(source_unit_name, image_key_hint, image_path, source_unit_dir)
    clip_stem = _derive_clip_stem_from_image_stem(image_path.stem)
    clip_path = _resolve_local_file(
        source_unit_dir,
    ...
    if clip_path is not None:
        clip_key = _normalize_unit_scoped_key(source_unit_name, None, clip_path, source_unit_dir)
...
def _write_failure_log(config: PipelineConfig, source_unit_name: str, rows: list[dict[str, Any]]) -> Path | None:
    if not rows:
        return None
    failed_dir = Path(config.manifest_dir) / "failed"
    failed_dir.mkdir(parents=True, exist_ok=True)
    path = failed_dir / f"prelabeled_import_{source_unit_name}_{_now():%Y%m%d_%H%M%S}.jsonl"
