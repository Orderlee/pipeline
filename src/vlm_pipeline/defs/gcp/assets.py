...
@asset(
    key=AssetKey(["pipeline", "gcs_download_to_incoming"]),
    description="GCS 다운로드를 /nas/incoming으로 적재 (버킷명 하위 폴더)",
    group_name="gcp",
    config_schema={
        ...
        "stall_seconds": Field(
            int,
            default_value=_as_int(os.getenv("GCS_STALL_SECONDS"), 300),
        ),
        "max_restarts": Field(
            int,
...
def gcs_download_to_incoming(context):
    ...
    timeout_sec = max(60, _as_int(cfg.get("timeout_sec"), 60 * 60 * 6))
