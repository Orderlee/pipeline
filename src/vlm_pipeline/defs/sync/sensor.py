...
def _read_table_snapshot(table_name: str) -> tuple[int, str | None]:
    ...
    retry_count = _int_env("DUCKDB_SENSOR_LOCK_RETRY_COUNT", 5, 0)
    retry_delay_ms = _int_env("DUCKDB_SENSOR_LOCK_RETRY_DELAY_MS", 200, 10)
    max_retry_delay_ms = _int_env("DUCKDB_SENSOR_LOCK_RETRY_MAX_DELAY_MS", 2000, retry_delay_ms)
    ...
    for attempt in range(retry_count + 1):
        ...
        try:
            ...
            if _is_lock_conflict(exc) and attempt < retry_count:
