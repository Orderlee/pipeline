...
def _run_ffprobe_with_retry(cmd: list[str], timeout_sec: int) -> subprocess.CompletedProcess:
    ...
    retry_count = _int_env("VIDEO_FFPROBE_RETRY_COUNT", 1, 0)
    retry_delay_ms = _int_env("VIDEO_FFPROBE_RETRY_DELAY_MS", 250, 0)
...
def _probe_frame_count_fallback(file_path: Path) -> int:
    ...
    timeout_sec = _int_env("VIDEO_FFPROBE_COUNT_TIMEOUT_SEC", 120, 10)
    cmd = [
        "ffprobe",
        "-v",
        "error",
...
def load_video_once(path: str | Path, include_file_stream: bool = False) -> dict:
    ...
    # ffprobe 1회 호출
    timeout_sec = _int_env("VIDEO_FFPROBE_TIMEOUT_SEC", 120, 10)
    cmd = [
        "ffprobe",
        "-v", "error",
        "-print_format", "json",
