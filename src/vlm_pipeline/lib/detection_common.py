...
# ---------------------------------------------------------------------------
# Spec bbox class resolution (bbox_labeling / sam3_bbox_labeling 공통)
# ---------------------------------------------------------------------------

@dataclass(frozen=True)
class SpecBboxClassResolution:
    """spec config에서 해석된 bbox detection 대상 클래스 정보."""
    target_classes: list[str]
    class_source: str
    resolved_config_id: str | None


def resolve_spec_bbox_classes(db, spec_id: str) -> SpecBboxClassResolution:
    """spec_id로부터 bbox detection 대상 클래스를 해석한다.

    bbox_labeling(YOLO)과 sam3_bbox_labeling(SAM3)에서 동일한 로직이므로 공통화한다.
    """
    config_bundle = load_persisted_spec_config(db, spec_id)
    resolved_config_id = str(config_bundle["resolved_config_id"] or "").strip() or None
    spec_classes = normalize_classes(config_bundle["spec"].get("classes") or [])
    ...
    return SpecBboxClassResolution(
