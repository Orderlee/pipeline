...
def _extract_frame(video_path: Path, sec: float, timeout_sec: int = 60) -> Image.Image | None:
...
def _load_places365_runtime() -> dict[str, Any] | None:
    ...
    use_places = bool_env("VIDEO_ENV_USE_PLACES365", True)
    if not use_places:
        return None
    ...
    auto_download = bool_env("VIDEO_ENV_AUTO_DOWNLOAD", True)
