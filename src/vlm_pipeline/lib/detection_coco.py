"""Utilities for detection payloads and single-image COCO conversion.

YOLO/SAM3 등 detection 모델의 raw 응답을 COCO format으로 변환하는 공통 헬퍼.
"""
...
# ---------------------------------------------------------------------------
# SAM3 detection → COCO 변환
# ---------------------------------------------------------------------------


def convert_sam3_detections_for_coco(sam_detections: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """SAM3 서버 응답의 detection 리스트를 build_coco_detection_payload 입력 형식으로 변환.

    SAM3 detection: {prompt_class, mask_bbox=[x1,y1,x2,y2], score, ...}
    COCO builder 입력: {class, bbox=[x1,y1,x2,y2], confidence}
    """
    converted: list[dict[str, Any]] = []
    for det in sam_detections:
        prompt_class = str(det.get("prompt_class") or "").strip().lower()
        mask_bbox = det.get("mask_bbox")
        if not prompt_class or not isinstance(mask_bbox, list) or len(mask_bbox) < 4:
            continue
        entry: dict[str, Any] = {
            "class": prompt_class,
            "bbox": [float(v) for v in mask_bbox[:4]],
        }
        score = det.get("score")
